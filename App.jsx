...
import {
  ...
  Wallet, TrendingUp, AlertTriangle, BookOpen,
} from "lucide-react";
...
function MainApp({ profile }) {
  ...
  const addStudent = async (s) => { const { data } = await supabase.from("students").insert({ ...s, school_id: schoolId }).select(); if (data) setStudents((p) => [...p, data[0]]); };
  ...
  return (
    <div className="min-h-screen w-full flex" style={{ background: COLORS.paper, fontFamily: "'Inter', sans-serif" }}>
      ...
      <main className="flex-1 p-5 md:p-10 pb-20 md:pb-10 max-w-5xl mx-auto w-full">
        {tab === "dashboard" && role === "parent" && <ParentChildrenView children={myChildren} paidByStudent={paidByStudent} currency={currency} />}
        ...
        {tab === "students" && (role === "comptable" || role === "fondateur") && (
          <StudentsView students={students} paidByStudent={paidByStudent} currency={currency} readOnly={role === "fondateur"} parents={parents} onAdd={() => setModal("student")} onDelete={deleteStudent} onPay={() => setModal("payment")} onLink={(s) => setLinkingStudent(s)} />
        )}
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard icon={AlertTriangle} label="Reste à payer" value={fmt(stats.reste, currency)} color={COLORS.negative} bg={COLORS.negativeSoft} />
...
function StudentsView({ students, paidByStudent, currency, readOnly, parents, onAdd, onDelete, onPay, onLink }) {
