...
function StudentFileMovementsView({ movements, students, onAdd, onDelete }) {
  ...
  const submit = async () => {
    ...
    setSaving(true);
    await onAdd({ student_id: studentId, movement_type: movementType, destination: destination.trim(), reason: reason.trim() || null, movement_date: movementDate });
    setSaving(false);
    setShowForm(false);
    setStudentId(""); setDestination(""); setReason("");
  };
