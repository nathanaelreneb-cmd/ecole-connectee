...
function MainApp({ profile, refreshProfile }) {
  ...
  const generatePromotionList = async (targetYear) => {
    ...
    const { data, error } = await supabase
      ...
      .select();
    if (data) {
      setPromotionEntries((prev) => [...prev, ...data.filter((d) => !prev.some((p) => p.id === d.id))]);
      notify(t("success_saved"));
    }
    return error;
  };
...
function CalendarView({ events, canManage, onAdd, onDelete }) {
  const { t, lang } = useLang();
          ...
          <div key={className} className="mb-6">
            ...
            <div className="space-y-2">
              {rows.map(({ entry, student }) => (
                <div key={entry.id} className="rounded-2xl p-4 flex items-center justify-between flex-wrap gap-3" style={{ background: COLORS.card, border: `1px solid ${COLORS.line}` }}>
                  <div>
  ...
  return (
    <div>
      ...
      {rows.length === 0 ? (
        ...
        <div className="space-y-2">
          {rows.map(({ entry, student }) => (
            <div key={entry.id} className="rounded-2xl p-4" style={{ background: COLORS.card, border: `1px solid ${COLORS.line}` }}>
              <div className="flex items-center justify-between flex-wrap gap-2 mb-2">
                <div>
                  <div className="font-semibold" style={{ color: student.gender === "Fille" ? COLORS.negative : COLORS.ink }}>{student.full_name}</div>
                  <div className="text-xs mt-0.5" style={{ color: COLORS.inkSoft }}>
                    {student.class_name} → {entry.target_class_name || t("needs_manual_class")} {entry.is_repeating && `(${t("repeating_label")})`}
                  </div>
                </div>
