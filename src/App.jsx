    ...
    <div className="min-h-screen w-full flex" style={{ background: COLORS.paper, fontFamily: "'Inter', sans-serif" }}>
      {isOffline && (
        <div className="fixed top-0 left-0 right-0 z-[70] flex items-center justify-center gap-2 px-4 py-2.5" style={{ background: COLORS.negative }}>
          ...
          <span className="text-sm font-semibold" style={{ color: "#fff" }}>{t("offline_banner")}</span>
        </div>
      )}
      {toast && (
    ...
    <Modal title={t("my_profile")} onClose={onClose}>
      <div className="space-y-5">
        <div className="flex flex-col items-center">
          <input ref={avatarInputRef} type="file" accept="image/*" className="hidden" onChange={(e) => { pickAvatar(e.target.files?.[0]); e.target.value = ""; }} />
          <input ref={avatarCameraRef} type="file" accept="image/*" capture="user" className="hidden" onChange={(e) => { pickAvatar(e.target.files?.[0]); e.target.value = ""; }} />
        ...
        {canSign && (
          <div>
            ...
            <input ref={signatureInputRef} type="file" accept="image/*" className="hidden" onChange={(e) => { pickSignature(e.target.files?.[0]); e.target.value = ""; }} />
            <input ref={signatureCameraRef} type="file" accept="image/*" capture="environment" className="hidden" onChange={(e) => { pickSignature(e.target.files?.[0]); e.target.value = ""; }} />
