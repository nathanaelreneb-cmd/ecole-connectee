...
export default function App() {
  ...
  const loadProfile = useCallback(async (userId) => {
    const { data } = await supabase.from("profiles").select("*, schools(name, code, currency, current_year, level_type, subscribed, subscription_tier, trial_started_at, momo_number, momo_provider, logo_url, card_theme)").eq("user_id", userId).maybeSingle();
...
function MainApp({ profile, refreshProfile }) {
  ...
  const { t, lang } = useLang();
  const NAV = {
    fondateur: [["dashboard", t("nav_dashboard"), LayoutDashboard], ["students", t("nav_students"), Users], ["studentcards", t("nav_student_cards"), CreditCard], ["annualbalance", t("nav_annual_balance"), TrendingUp], ["promotion", t("nav_promotion"), ArrowRight], ["filemovements", t("nav_file_movements"), ArrowRightLeft], ["tuitionfees", t("nav_tuition_fees"), Coins], ["services", t("nav_services"), Package], ["attendance", t("nav_attendance"), CalendarCheck], ["grades", t("nav_grades"), ClipboardList], ["expenses", t("nav_expenses"), Receipt], ["staff", t("nav_staff"), Wallet], ["mysalary", t("nav_my_salary"), Banknote], ["calendar", t("nav_calendar"), CalendarCheck]],
    comptable: [["students", t("nav_students_payments"), Users], ["paymentdeclarations", t("nav_payment_declarations"), Smartphone], ["reenrollment", t("nav_reenrollment"), ArrowRight], ["services", t("nav_services"), Package], ["weeklyjournal", t("nav_weekly_journal"), BookMarked], ["dashboard", t("nav_stats"), LayoutDashboard], ["expenses", t("nav_expenses"), Receipt], ["staff", t("nav_staff"), Wallet], ["mysalary", t("nav_my_salary"), Banknote], ["calendar", t("nav_calendar"), CalendarCheck]],
    ...
    <div className="min-h-screen w-full flex" style={{ background: COLORS.paper, fontFamily: "'Inter', sans-serif" }}>
      ...
      <main className="flex-1 p-5 md:p-10 pb-20 md:pb-10 max-w-5xl mx-auto w-full">
        ...
        {tab === "tuitionfees" && role === "fondateur" && (
          <TuitionFeesView levelType={levelType} tuitionFees={tuitionFees} onSave={saveTuitionFee} tierComplete={hasTierAccess(profile.schools, "complet")} schoolMomo={profile.schools} onSaveMomo={saveMomoSettings} onUploadLogo={uploadSchoolLogo} onSaveTheme={saveCardTheme} />
        )}
...
function StudentCardPreview({ schoolName, schoolLogo, currentYear, student, directeur, theme, onClose }) {
  const { t } = useLang();
          ...
          <div className="rounded-xl overflow-hidden" style={{ background: "#fff" }}>
            ...
            <div className="p-4">
              ...
              <div className="flex gap-3 mb-3">
                ...
                <div className="flex-1 min-w-0 space-y-1.5">
                  <div>
                    ...
                    <div className="inline-block text-sm font-bold pb-0.5" style={{ color: COLORS.ink, borderBottom: `2px solid ${th.from}` }}>{student.full_name}</div>
                  </div>
                  <div>
              ...
              <div className="flex items-center justify-between pt-3" style={{ borderTop: `1px dashed ${COLORS.line}` }}>
                <div className="text-[10px]" style={{ color: COLORS.inkSoft }}>{t("director_label")}</div>
...
function MiniStudentCard({ schoolName, schoolLogo, currentYear, student, directeur, theme }) {
  const { t } = useLang();
  const genderColor = student.gender === "Fille" ? "#C1502E" : "#20304A";
  const th = CARD_THEMES[theme] || CARD_THEMES.corail;
  const gradient = `linear-gradient(90deg, ${th.from}, ${th.to})`;
  ...
  return (
    <div className="rounded-xl overflow-hidden p-[3px]" style={{ background: gradient, breakInside: "avoid" }}>
      <div className="rounded-lg overflow-hidden" style={{ background: "#fff" }}>
        <div className="px-2.5 py-1.5 flex items-center gap-1.5" style={{ background: COLORS.ink }}>
          <div className="w-6 h-6 rounded-full overflow-hidden shrink-0 flex items-center justify-center" style={{ background: "#fff" }}>
            {schoolLogo ? <img src={schoolLogo} alt="" className="w-full h-full object-contain" /> : <div className="w-full h-full rounded-full" style={{ background: gradient }} />}
          </div>
          <div className="text-center flex-1 min-w-0">
            <div className="text-[6px] uppercase tracking-widest font-semibold truncate" style={{ color: "rgba(255,255,255,0.6)" }}>{t("republic_header")}</div>
            <div className="text-[9px] font-extrabold truncate" style={{ color: "#fff", fontFamily: "'Manrope', sans-serif" }}>{t("school_card_title")}</div>
          </div>
        </div>
        <div className="p-2">
          <div className="text-center mb-1.5">
            <div className="text-[10px] font-extrabold truncate" style={{ color: COLORS.ink, fontFamily: "'Manrope', sans-serif" }}>{schoolName}</div>
            <div className="text-[8px] font-medium" style={{ color: th.from }}>{currentYear}</div>
          </div>
          <div className="flex gap-1.5 mb-1.5">
            <div className="shrink-0 rounded-md overflow-hidden" style={{ width: 44, height: 54, background: COLORS.paper, border: `2px solid ${genderColor}` }}>
              {student.photo_url ? <img src={student.photo_url} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center"><Users size={14} style={{ color: COLORS.inkSoft }} /></div>}
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-[8px] font-bold truncate" style={{ color: COLORS.ink, borderBottom: `1.5px solid ${th.from}` }}>{student.full_name}</div>
              <div className="text-[7px] font-semibold mt-1 px-1 py-0.5 rounded-full inline-block" style={{ background: `${th.from}22`, color: th.from }}>{student.class_name}</div>
              <div className="text-[7px] mt-1" style={{ color: genderColor, fontWeight: 600 }}>{student.gender || "—"}</div>
            </div>
          </div>
          <div className="text-[6.5px] leading-tight p-1 rounded-md mb-1.5" style={{ background: COLORS.paper, color: COLORS.ink }}>
          ...
          <div className="flex items-center justify-between pt-1" style={{ borderTop: `1px dashed ${COLORS.line}` }}>
            <div className="text-[6px]" style={{ color: COLORS.inkSoft }}>{t("director_label")}</div>
            <div className="text-[7px] font-semibold" style={{ color: COLORS.ink }}>{formatSignerName(directeur, t)}</div>
          </div>
        </div>
      </div>
    </div>
...
function ClassCardsSheetView({ students, schoolName, schoolLogo, cardTheme, currentYear, directeur, className, onClose }) {
  const { t } = useLang();
  const [downloading, setDownloading] = useState(false);
  const [error, setError] = useState("");
      ...
      <div className="min-h-screen p-3 sm:p-6">
        ...
        <div id="sheet-print-area" className="rounded-2xl p-4" style={{ background: "#fff" }}>
          <div className="grid grid-cols-3 gap-3">
            {students.map((s) => (
              <MiniStudentCard key={s.id} schoolName={schoolName} schoolLogo={schoolLogo} currentYear={currentYear} student={s} directeur={directeur} theme={cardTheme} />
            ))}
          </div>
        </div>
      </div>
...
function StudentCardsView({ students, schoolName, schoolLogo, cardTheme, currentYear, directeur, onUploadPhoto }) {
  const { t } = useLang();
  const [search, setSearch] = useState("");
  const classNames = useMemo(() => [...new Set(students.map((s) => s.class_name).filter(Boolean))].sort(), [students]);
  const [selectedClass, setSelectedClass] = useState("");
  ...
  return (
    <div>
      ...
      {cardFor && (
        <StudentCardPreview schoolName={schoolName} schoolLogo={schoolLogo} currentYear={currentYear} student={cardFor} directeur={directeur} theme={cardTheme} onClose={() => setCardFor(null)} />
      )}
      {showSheet && (
        <ClassCardsSheetView
...
function TuitionFeesView({ levelType, tuitionFees, onSave, tierComplete, schoolMomo, onSaveMomo, onUploadLogo, onSaveTheme }) {
  const { t } = useLang();
...
function ParentChildrenView({ children, paidByStudent, currency, payments, onRelink, grades, attendance, staffProfiles, viewYear, setViewYear, availableYears, currentYear, schoolName, bulletinAppreciations, disciplineNotes }) {
  const { t, lang } = useLang();
...
function StudentModal({ levels, student, existingClassNames, tuitionFees, role, tierComplete, onClose, onSave, onUpdate, onUploadPhoto }) {
  ...
  const handlePhotoPick = async (file) => {
    ...
    await onUploadPhoto(student.id, file);
