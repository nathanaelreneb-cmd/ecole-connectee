...
export default function App() {
  ...
  const loadProfile = useCallback(async (userId) => {
    const { data } = await supabase.from("profiles").select("*, schools(name, code, currency, current_year, level_type, subscribed, subscription_tier, trial_started_at, momo_number, momo_provider, logo_url, card_theme, director_title, ire, dpe, dsee)").eq("user_id", userId).maybeSingle();
...
function MainApp({ profile, refreshProfile, userEmail }) {
  ...
  const loadAll = useCallback(async () => {
    try {
      const [s, p, l, h, a, par, exp, gr, rel, staff, sp, rem, att, ta, cht, subj, grs, tf, svc, svcSub, svcPay, appr, promo, cal, disc, pdecl, tt, fmov, wj] = await Promise.all([
      ...
      const allSucceeded = [s, p, l, h, a, par, exp, gr, rel, staff, sp, rem, att, ta, cht, subj, grs, tf, svc, svcSub, svcPay, appr, promo, cal, disc, pdecl, tt, fmov, wj].every((r) => r.data);
      if (allSucceeded) {
        const snapshot = {
  ...
  const saveAdminInfo = async (ire, dpe, dsee) => {
    const { error } = await supabase.from("schools").update({ ire: ire || null, dpe: dpe || null, dsee: dsee || null }).eq("id", schoolId);
    if (!error) { await refreshProfile(); notify(t("success_saved")); }
  };
  ...
  const { t, lang } = useLang();
  const NAV = {
    fondateur: [["dashboard", t("nav_dashboard"), LayoutDashboard], ["students", t("nav_students"), Users], ["studentcards", t("nav_student_cards"), CreditCard], ["staffcards", t("nav_staff_cards"), CreditCard], ["annualbalance", t("nav_annual_balance"), TrendingUp], ["promotion", t("nav_promotion"), ArrowRight], ["filemovements", t("nav_file_movements"), ArrowRightLeft], ["tuitionfees", t("nav_tuition_fees"), Coins], ["services", t("nav_services"), Package], ["attendance", t("nav_attendance"), CalendarCheck], ["grades", t("nav_grades"), ClipboardList], ["expenses", t("nav_expenses"), Receipt], ["staff", t("nav_staff"), Wallet], ["mysalary", t("nav_my_salary"), Banknote], ["calendar", t("nav_calendar"), CalendarCheck]],
    comptable: [["students", t("nav_students_payments"), Users], ["dashboard", t("nav_stats"), LayoutDashboard], ["paymentdeclarations", t("nav_payment_declarations"), Smartphone], ["reenrollment", t("nav_reenrollment"), ArrowRight], ["services", t("nav_services"), Package], ["weeklyjournal", t("nav_weekly_journal"), BookMarked], ["expenses", t("nav_expenses"), Receipt], ["staff", t("nav_staff"), Wallet], ["mysalary", t("nav_my_salary"), Banknote], ["calendar", t("nav_calendar"), CalendarCheck]],
    enseignant: isSecondaire
      ? [["grades", t("nav_grades"), ClipboardList], ["myassignments", t("nav_my_assignments"), BookMarked], ["attendance", t("nav_attendance"), CalendarCheck], ["lessonai", t("nav_lesson_ai"), Sparkles], ["lessons", t("nav_lessons"), BookOpen], ["discipline", t("nav_discipline"), AlertTriangle], ["timetable", t("nav_timetable"), CalendarCheck], ["weeklyjournal", t("nav_weekly_journal"), BookMarked], ["students", t("nav_students"), Users], ["mysalary", t("nav_my_salary"), Banknote], ["calendar", t("nav_calendar"), CalendarCheck]]
      : [["grades", t("nav_grades"), ClipboardList], ["attendance", t("nav_attendance"), CalendarCheck], ["lessonai", t("nav_lesson_ai"), Sparkles], ["lessons", t("nav_lessons"), BookOpen], ["discipline", t("nav_discipline"), AlertTriangle], ["timetable", t("nav_timetable"), CalendarCheck], ["weeklyjournal", t("nav_weekly_journal"), BookMarked], ["students", t("nav_students"), Users], ["mysalary", t("nav_my_salary"), Banknote], ["calendar", t("nav_calendar"), CalendarCheck]],
    parent: [["dashboard", t("nav_my_children"), GraduationCap], ["revision", t("nav_revision"), Sparkles], ["lessons", t("nav_lessons"), BookOpen], ["timetable", t("nav_timetable"), CalendarCheck], ["announcements", t("nav_announcements"), Megaphone], ["remarks", t("nav_my_remarks"), MessageSquare], ["pay", t("nav_pay_remotely"), Smartphone], ["calendar", t("nav_calendar"), CalendarCheck]],
    personnel: [["mysalary", t("nav_my_salary"), Banknote], ["weeklyjournal", t("nav_weekly_journal"), BookMarked], ["announcements", t("nav_announcements"), Megaphone], ["calendar", t("nav_calendar"), CalendarCheck]],
    directeur: [["grades", t("nav_grades"), ClipboardList], ["timetable", t("nav_timetable"), CalendarCheck], ["dossiers", t("nav_dossiers"), Contact], ["studentcards", t("nav_student_cards"), CreditCard], ["staffcards", t("nav_staff_cards"), CreditCard], ["filemovements", t("nav_file_movements"), ArrowRightLeft], ["weeklyjournal", t("nav_weekly_journal"), BookMarked], ["discipline", t("nav_discipline"), AlertTriangle], ["promotion", t("nav_promotion"), ArrowRight], ["announcements", t("nav_announcements"), Megaphone], ["attendance", t("nav_attendance"), CalendarCheck], ...(isSecondaire ? [["classteachers", t("nav_head_teachers"), Shield]] : [["classassign", t("nav_class_assign"), Shield]]), ["lessons", t("nav_lessons"), BookOpen], ["mysalary", t("nav_my_salary"), Banknote], ["remarks", t("nav_remarks"), MessageSquare], ["calendar", t("nav_calendar"), CalendarCheck]],
  }[role];
      ...
      <main className="flex-1 p-5 md:p-10 pb-20 md:pb-10 max-w-5xl mx-auto w-full">
        ...
        {tab === "grades" && isSecondaire && (role === "enseignant" || role === "fondateur" || role === "directeur") && (
          <>
            ...
            directorTitle={profile.schools?.director_title}
            schoolInfo={{ name: profile.schools?.name, ire: profile.schools?.ire, dpe: profile.schools?.dpe, dsee: profile.schools?.dsee, currentYear }}
          />
          </>
        )}
        ...
        {tab === "tuitionfees" && role === "fondateur" && (
          <TuitionFeesView levelType={levelType} tuitionFees={tuitionFees} onSave={saveTuitionFee} tierComplete={hasTierAccess(profile.schools, "complet")} schoolMomo={profile.schools} onSaveMomo={saveMomoSettings} onUploadLogo={uploadSchoolLogo} onSaveTheme={saveCardTheme} onSaveDirectorTitle={saveDirectorTitle} onSaveAdminInfo={saveAdminInfo} />
        )}
        ...
        {tab === "paymentdeclarations" && role === "comptable" && (
...
async function replacePhotoBackground(file, bgColor = "#ffffff", maxDim = 640) {
  ...
  try {
    ...
    const resultCanvas = await new Promise((resolve, reject) => {
      const timeout = setTimeout(() => reject(new Error("délai dépassé")), 15000);
...
function writeOfficialHeader(sheet, schoolInfo, className, termLabel, span) {
  ...
  for (let i = 0; i < maxLines; i++) {
    ...
    row.getCell(1).font = { size: 10, color: { argb: "FF20304A" } };
    row.getCell(1).alignment = { horizontal: "left", vertical: "middle" };
    if (rightLines[i]) {
      sheet.mergeCells(row.number, halfSpan + 1, row.number, colSpan);
      const rightCell = row.getCell(halfSpan + 1);
      rightCell.value = rightLines[i];
      rightCell.font = { size: 10, bold: i === 0, color: { argb: "FF20304A" } };
...
function addStyledSheet(workbook, { title, headers, rows, boldRows = [], nameColIndex = 1, alwaysRedCols = [], schoolInfo, className, termLabel }) {
  const sheet = workbook.addWorksheet((title || "Feuille1").replace(/[\\/*?:[\]]/g, "").slice(0, 31));

  if (schoolInfo && termLabel) writeOfficialHeader(sheet, schoolInfo, className, termLabel, headers.length);
...
function PaymentDeclarationsView({ declarations, students, onConfirm, onReject }) {
  const { t, lang } = useLang();
  ...
  const DeclCard = ({ d }) => (
    <div className="rounded-2xl p-4" style={{ background: COLORS.card, border: `1px solid ${COLORS.line}` }}>
      ...
      <div className="text-sm mb-3" style={{ color: COLORS.inkSoft }}>{t("momo_reference_label")} : <span className="font-medium" style={{ color: COLORS.ink }}>{d.reference_code || "—"}</span></div>
...
function StudentCardsView({ students, schoolName, schoolLogo, cardTheme, currentYear, directeur, directorTitle, onUploadPhoto }) {
  ...
  return (
    <div>
      ...
      {cardFor && (
        <StudentCardPreview schoolName={schoolName} schoolLogo={schoolLogo} currentYear={currentYear} student={cardFor} directeur={directeur} theme={cardTheme} directorTitle={directorTitle} onClose={() => setCardFor(null)} />
      )}
      {showSheet && (
        <ClassCardsSheetView
        ...
        <div className="space-y-2">
          {classStudents.map((s) => (
            <div key={s.id} className="flex items-center justify-between px-4 py-3 rounded-xl" style={{ background: COLORS.card, border: `1px solid ${COLORS.line}` }}>
              <button onClick={() => setCardFor(s)} className="flex items-center gap-3 flex-1 min-w-0 text-left">
                <div className="w-10 h-10 rounded-lg overflow-hidden shrink-0" style={{ background: COLORS.paper }}>
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                ...
                <button onClick={() => setCardFor(s)} className="p-2 rounded-lg" style={{ background: COLORS.primarySoft, color: COLORS.primary }}>
                  <CreditCard size={15} />
                </button>
              </div>
            </div>
...
function AnnualBalanceView({ payments, servicePayments, services, expenses, currency, viewYear }) {
  const { t } = useLang();
...
function TuitionFeesView({ levelType, tuitionFees, onSave, tierComplete, schoolMomo, onSaveMomo, onUploadLogo, onSaveTheme, onSaveDirectorTitle, onSaveAdminInfo }) {
  const { t } = useLang();
  ...
  const [directorTitle, setDirectorTitle] = useState(schoolMomo?.director_title || "Directeur");
  const [savingDirectorTitle, setSavingDirectorTitle] = useState(false);
  const [ire, setIre] = useState(schoolMomo?.ire || "");
  const [dpe, setDpe] = useState(schoolMomo?.dpe || "");
  const [dsee, setDsee] = useState(schoolMomo?.dsee || "");
  ...
  const handleSaveAdminInfo = async () => {
    ...
    await onSaveAdminInfo(ire.trim(), dpe.trim(), dsee.trim());
      ...
      <div className="rounded-2xl p-5 mb-8" style={{ background: COLORS.card, border: `1px solid ${COLORS.line}` }}>
        ...
        <div className="flex items-center gap-3">
          <div className="w-16 h-16 rounded-xl overflow-hidden shrink-0" style={{ background: COLORS.paper, border: `1px solid ${COLORS.line}` }}>
            {schoolMomo?.logo_url ? <img src={schoolMomo.logo_url} alt="" className="w-full h-full object-contain" /> : <div className="w-full h-full flex items-center justify-center"><ImageIcon size={20} style={{ color: COLORS.inkSoft }} /></div>}
          </div>
          <button onClick={() => logoInputRef.current?.click()} disabled={uploadingLogo} className="text-xs font-semibold px-3 py-2.5 rounded-lg flex items-center gap-1.5" style={{ background: COLORS.primary, color: "#fff" }}>
            {uploadingLogo && <Loader2 className="animate-spin" size={12} />} {schoolMomo?.logo_url ? t("change_photo") : t("add_logo_button")}
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {Object.entries(CARD_THEMES).map(([key, th]) => (
            <button
              ...
              style={{ border: (schoolMomo?.card_theme || "corail") === key ? `2px solid ${th.from}` : `1px solid ${COLORS.line}` }}
            >
...
function GradesSecondaireView({ students, gradesSecondaire, releases, allAssignments, myUserId, classHeadTeachers, subjectsSecondaire, currency, readOnly, canPublish, staffProfiles, isHeadTeacherOf, onSave, onSetCoefficient, onToggleRelease, onPublishAll, schoolName, bulletinAppreciations, onSaveAppreciation, directorTitle, schoolInfo }) {
  ...
  const renderBulletin = () => {
    ...
    const buildBulletinWorkbook = async () => {
      ...
      const headers = [t("order_number"), t("student"), ...allSubjects, t("general_average"), t("rank")];
      const rowsData = rows.map((row, i) => ({
        cells: [i + 1, row.student.full_name, ...row.subjectAvgs.map((sa) => (sa.avg !== null ? sa.avg.toFixed(2) : "")), row.moyenneGenerale !== null ? row.moyenneGenerale.toFixed(2) : "", bulletinRanks[i]],
      ...
      addStyledSheet(workbook, {
        ...
        rows: rowsData,
        nameColIndex: 2,
        alwaysRedCols: [headers.length - 1, headers.length],
        schoolInfo,
        className,
    ...
    const buildAllBulletinsWorkbook = async () => {
      ...
      for (const cName of classNames) {
        ...
        const headers = [t("order_number"), t("student"), ...cSubjects, t("general_average"), t("rank")];
        const rowsData = cRows.map((row, i) => ({
          cells: [i + 1, row.student.full_name, ...row.subjectAvgs.map((sa) => (sa.avg !== null ? sa.avg.toFixed(2) : "")), row.moyenneGenerale !== null ? row.moyenneGenerale.toFixed(2) : "", allBulletinRanks[i]],
          isRed: row.student.gender === "Fille",
        }));
        addStyledSheet(workbook, {
          title: `${cName} · ${term}`,
...
function GradesView({ students, grades, releases, readOnly, canPublish, lockedClass, passThreshold, staffProfiles, onSave, onToggleRelease, onPublishAll, schoolName, bulletinAppreciations, onSaveAppreciation, directorTitle, schoolInfo }) {
  ...
  const buildClassSheet = async (workbook, cName, cTerm) => {
    ...
    if (schoolInfo) writeOfficialHeader(sheet, schoolInfo, cName, termLabelFor(cTerm, schoolInfo?.currentYear), 6 + cSubjects.length);
    ...
    styleHeaderRow(sheet.addRow(["N°", "Élève", "Sexe", ...cSubjects, "Total", "Moyenne", "Rang"]));
    const moyenneColIdx = 5 + cSubjects.length;
    const rangColIdx = 6 + cSubjects.length;
    cRows.forEach((r, i) => {
      const row = sheet.addRow([i + 1, r.student.full_name, r.student.gender || "—", ...cSubjects.map((s) => cGetScore(r.student.id, s) ?? ""), r.total.toFixed(1), r.moyenne !== null ? r.moyenne.toFixed(2) : "", cRanks[i]]);
      styleDataRow(row);
...
function ParentPayView({ comptable, currency, schoolMomo, schoolName, children, declarations, onDeclare }) {
  const { t, lang } = useLang();
  const [copied, setCopied] = useState(false);
  const [showForm, setShowForm] = useState(false);
  ...
  const submit = async () => {
    ...
    setSaving(true);
    await onDeclare({ student_id: studentId, amount: parseFloat(amount), reference_code: referenceCode.trim() || null });
    setSaving(false);
    setShowForm(false);
    setAmount(""); setReferenceCode("");
  };
        ...
        <div className="mb-6">
          ...
          <div className="space-y-2">
            {declarations.map((d) => {
              ...
              return (
                <div key={d.id} className="rounded-xl p-3.5 flex items-center justify-between" style={{ background: COLORS.card, border: `1px solid ${COLORS.line}` }}>
                  <div>
...
function StudentModal({ levels, student, existingClassNames, tuitionFees, role, tierComplete, onClose, onSave, onUpdate, onUploadPhoto }) {
  ...
  const handlePhotoPick = async (file) => {
    ...
    if (bgReplaceEnabled) {
      ...
      const processed = await replacePhotoBackground(file, bgReplaceColor);
      toUpload = processed;
      setBgReplaceNotice(processed !== file ? t("bg_replace_done") : t("bg_replace_fallback"));
    }
    await onUploadPhoto(student.id, toUpload);
        ...
        {isEdit && (
          <div>
            ...
            <div className="rounded-xl p-3 flex items-center gap-3 mb-2" style={{ background: COLORS.paper }}>
              {student?.photo_url ? (
                <img src={student.photo_url} alt="" className="w-16 h-16 rounded-lg object-cover" />
              ) : (
            ...
            <label className="flex items-center gap-2.5 px-3 py-2 rounded-lg cursor-pointer" style={{ background: bgReplaceEnabled ? COLORS.primarySoft : COLORS.paper }}>
              ...
              {bgReplaceEnabled && (
                <select value={bgReplaceColor} onChange={(e) => setBgReplaceColor(e.target.value)} onClick={(e) => e.stopPropagation()} className="text-xs px-2 py-1 rounded-lg outline-none" style={inputStyle}>
            ...
            <p className="text-xs mt-1" style={{ color: COLORS.inkSoft }}>{t("bg_replace_experimental_note")}</p>
          </div>
        )}
          ...
          {SUBSCRIPTION_TIERS.map((tier) => (
            <div key={tier.key} className="rounded-xl p-4 flex items-center justify-between gap-3" style={{ border: `1px solid ${COLORS.line}` }}>
              ...
              <div className="text-right shrink-0">
                ...
                <div className="text-xs" style={{ color: COLORS.inkSoft }}>{t("per_month")}</div>
              </div>
            </div>
          ))}
        </div>
