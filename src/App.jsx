      ...
      <main className="flex-1 p-5 md:p-10 pb-20 md:pb-10 max-w-5xl mx-auto w-full">
        {tab === "dashboard" && role === "parent" && (
          <ParentChildrenView
            ...
            payments={payments}
            onRelink={relinkSelf}
            grades={grades.filter((g) => releases.some((r) => r.class_name === g.class_name && r.term === g.term && r.published))}
          />
        )}
        ...
        {tab === "grades" && (role === "enseignant" || role === "fondateur") && (
          <GradesView students={students} grades={grades} releases={releases} readOnly={role === "fondateur"} onSave={saveGrade} onToggleRelease={toggleRelease} />
        )}
        {tab === "announcements" && (
...
// ---------- Notes (enseignant) ----------
function GradesView({ students, grades, releases, readOnly, onSave, onToggleRelease }) {
  ...
  const classGrades = useMemo(() => grades.filter((g) => g.class_name === className && g.term === term), [grades, className, term]);
            ...
            return (
              <div key={c.id} className="rounded-2xl p-5" style={{ background: COLORS.card, border: `1px solid ${COLORS.line}` }}>
                ...
                {(() => {
                  ...
                  const byTerm = {};
                  childGrades.forEach((g) => { (byTerm[g.term] = byTerm[g.term] || []).push(g); });
                  return (
                    <div className="mt-4 pt-3" style={{ borderTop: `1px solid ${COLORS.line}` }}>
                      <div className="text-xs font-semibold uppercase tracking-wider mb-2" style={{ color: COLORS.inkSoft }}>Notes (bulletin publié)</div>
                      {Object.entries(byTerm).map(([term, tGrades]) => {
                        const avg = tGrades.reduce((s, g) => s + Number(g.score), 0) / tGrades.length;
                        return (
                          <div key={term} className="mb-3">
                            <div className="flex items-center justify-between mb-1.5">
                              <span className="text-xs font-medium" style={{ color: COLORS.ink }}>{term}</span>
