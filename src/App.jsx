...
function OnboardingScreen({ userId, onDone, onBack }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const { error: profileError } = await supabase.from("profiles").insert({ user_id: userId, school_id: schoolId, role, full_name: fullName.trim(), phone: phone.trim() || null, class_name: role === "enseignant" ? className.trim() || null : null });
...
function MainApp({ profile }) {
  ...
  const loadAll = useCallback(async () => {
    const [s, p, l, h, a, par, exp, gr, rel] = await Promise.all([
