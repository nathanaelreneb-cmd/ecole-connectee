...
function MainApp({ profile, refreshProfile }) {
  ...
  const shareApp = async () => {
    const message = t("share_message")
      ...
      .replace("{code}", profile.schools?.code || "");
    const shareData = { title: "École Connectée", text: message, url: window.location.origin };
    if (navigator.share) {
      try { await navigator.share(shareData); } catch (e) {}
    } else {
      navigator.clipboard?.writeText(`${message} ${shareData.url}`);
      alert(t("link_copied"));
    }
  };
