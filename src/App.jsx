...
function LessonAIView({ subjectsOptions, classOptions, lang }) {
  ...
  const generate = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/generate-lesson", {
        ...
        body: JSON.stringify({ subject, className, lessonTitle, textbook, page, language: lang }),
      });
      const data = await res.json();
      if (!res.ok) { setError(data.error || t("ai_generic_error")); setLoading(false); return; }
      setResult(data);
    } catch (e) {
  ...
  return (
    <div>
      ...
      <p className="mb-6" style={{ color: COLORS.inkSoft }}>{t("lesson_ai_subtitle")}</p>
