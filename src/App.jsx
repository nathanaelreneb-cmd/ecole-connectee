...
function ExamComposerTab({ subjectsOptions, classOptions, lang }) {
  const { t } = useLang();
  const [subject, setSubject] = useState("");
  const [className, setClassName] = useState("");
    ...
    <div>
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {tabs.map(([key, label]) => (
          <button
            key={key}
            onClick={() => { setTab(key); if (key === "historique") setHistoryKey((k) => k + 1); }}
      ...
      {tab === "sujet" && <ExamComposerTab subjectsOptions={subjectsOptions} classOptions={classOptions} lang={lang} />}
    </div>
  );
}
