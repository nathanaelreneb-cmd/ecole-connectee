        ...
        <div className="rounded-2xl p-5 mb-8" style={{ background: COLORS.card, border: `1px solid ${COLORS.line}` }}>
          ...
          <div className="space-y-3 mb-3">
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              <div>
                <FieldLabel>{t("subject_label")}</FieldLabel>
                <input value={assignSubject} onChange={(e) => setAssignSubject(e.target.value)} placeholder={t("subject_placeholder2")} list="subject-suggestions-assign" className="w-full px-3 py-2.5 rounded-lg outline-none text-sm" style={inputStyle} />
                <datalist id="subject-suggestions-assign">{(existingSubjectNames || []).map((s) => <option key={s} value={s} />)}</datalist>
              </div>
              <div>
                <FieldLabel>{t("class_label")}</FieldLabel>
                <input value={assignClass} onChange={(e) => setAssignClass(e.target.value)} placeholder={t("class_placeholder2")} list="class-suggestions-assign" className="w-full px-3 py-2.5 rounded-lg outline-none text-sm" style={inputStyle} />
                <datalist id="class-suggestions-assign">{(existingClassNames || []).map((c) => <option key={c} value={c} />)}</datalist>
              </div>
            </div>
          </div>
    ...
    <div>
      ...
      <div className="rounded-2xl p-5 mb-6" style={{ background: COLORS.card, border: `1px solid ${COLORS.line}` }}>
        <div className="grid sm:grid-cols-2 gap-3 mb-3">
          <div>
            <FieldLabel>{t("subject_label")}</FieldLabel>
            <input value={subject} onChange={(e) => setSubject(e.target.value)} placeholder="Ex: Mathématiques" list="subject-suggestions-my" className="w-full px-3 py-2.5 rounded-lg outline-none" style={inputStyle} />
            <datalist id="subject-suggestions-my">{(existingSubjectNames || []).map((s) => <option key={s} value={s} />)}</datalist>
          </div>
          <div>
            <FieldLabel>{t("class_label")}</FieldLabel>
            <input value={className} onChange={(e) => setClassName(e.target.value)} placeholder="Ex: 6ème A" list="class-suggestions-my" className="w-full px-3 py-2.5 rounded-lg outline-none" style={inputStyle} />
            <datalist id="class-suggestions-my">{(existingClassNames || []).map((c) => <option key={c} value={c} />)}</datalist>
          </div>
        </div>
  ...
  return (
    <Modal title={isEdit ? t("edit_student") : t("new_student")} onClose={onClose}>
      <div className="space-y-4">
        ...
        <div>
          <FieldLabel>{t("class_label")}</FieldLabel>
          <input value={className} onChange={(e) => setClassName(e.target.value)} placeholder="Ex: CM2, 6ème A..." list="class-suggestions-student" className="w-full px-3 py-2.5 rounded-lg outline-none" style={inputStyle} />
          <datalist id="class-suggestions-student">
            {(existingClassNames || []).map((c) => <option key={c} value={c} />)}
          </datalist>
        </div>
