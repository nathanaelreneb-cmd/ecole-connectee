...
export default function App() {
  ...
  const loadProfile = useCallback(async (userId) => {
    const { data } = await supabase.from("profiles").select("*, schools(name, code, currency, current_year, level_type, subscribed, subscription_tier, trial_started_at, momo_number, momo_provider, logo_url, emblem_url, card_theme, director_title, ire, dpe, dsee)").eq("user_id", userId).maybeSingle();
...
function MainApp({ profile, refreshProfile, userEmail }) {
  ...
  const uploadSchoolEmblem = async (file) => {
    ...
    try {
      ...
      if (file.size > 15_000_000) { alert("Cette image est trop lourde. Choisis-en une plus légère."); return null; }
      const resized = await resizeImageMemorySafe(file, 500, 0.9);
      const path = `${schoolId}/emblem.jpg`;
      const { error: upErr } = await supabase.storage.from("school-logos").upload(path, resized, { upsert: true, contentType: "image/jpeg" });
      if (upErr) return null;
      const { data } = supabase.storage.from("school-logos").getPublicUrl(path);
      const url = `${data.publicUrl}?t=${Date.now()}`;
      ...
      <main className="flex-1 p-5 md:p-10 pb-20 md:pb-10 max-w-5xl mx-auto w-full">
        ...
        {tab === "grades" && isSecondaire && (role === "enseignant" || role === "fondateur" || role === "directeur") && (
          <>
            ...
            directorTitle={profile.schools?.director_title}
            schoolInfo={{ name: profile.schools?.name, ire: profile.schools?.ire, dpe: profile.schools?.dpe, dsee: profile.schools?.dsee, emblem_url: profile.schools?.emblem_url, currentYear }}
          />
          </>
        )}
...
async function writeOfficialHeader(workbook, sheet, schoolInfo, className, termLabel, span) {
  ...
  for (let i = 0; i < maxLines; i++) {
    ...
    if (rightLines[i]) {
      ...
      // "République de Guinée" ressort bien en gros caractère gras ; la devise en dessous
      // reste petite, comme un sous-titre.
      rightCell.font = i === 0
        ? { bold: true, size: 13, color: { argb: "FF20304A" } }
        : { size: 9, color: { argb: "FF20304A" } };
...
function SchoolSettingsView({ levelType, schoolInfo, onUploadLogo, onUploadEmblem, onSaveTheme, onSaveDirectorTitle, onSaveAdminInfo }) {
  const { t } = useLang();
