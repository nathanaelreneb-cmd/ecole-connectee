...
import {
  ...
  Wallet, TrendingUp, AlertTriangle, BookOpen, FileSpreadsheet, Smartphone, TrendingDown, Receipt, Shield, ClipboardList, ArrowDownWideNarrow, MessageSquare, Menu, CalendarCheck, Languages, Share2, Banknote, Contact, BookMarked, QrCode, Eye, EyeOff, Sparkles, Package, Coins, Download, Printer,
} from "lucide-react";
      ...
      <div className="min-h-screen flex items-start justify-center p-3 sm:p-6">
        <div className="w-full max-w-2xl">
          <div className="flex justify-end gap-2 mb-2 no-print flex-wrap">
            <button onClick={() => window.print()} className="flex items-center gap-1.5 px-4 py-2.5 rounded-lg font-semibold text-sm" style={{ background: COLORS.primary, color: "#fff" }}>
              <Download size={15} /> {t("download_pdf_button")}
            </button>
          ...
          <p className="text-xs mb-3 no-print" style={{ color: COLORS.inkSoft }}>{t("download_hint")}</p>
          ...
          <div id="bulletin-print-area" className="rounded-2xl p-7 sm:p-10" style={{ background: "#fff", boxShadow: "0 10px 40px rgba(0,0,0,0.2)" }}>
            ...
            <div className="grid grid-cols-2 gap-6 pt-5" style={{ borderTop: `1px solid ${COLORS.line}` }}>
              <div className="text-center">
                <div className="text-xs font-semibold mb-8" style={{ color: COLORS.inkSoft }}>{t("director_label")}</div>
                {directeur?.signature_url && <img src={directeur.signature_url} alt="" className="h-12 mx-auto object-contain mb-1" />}
                <div className="text-sm font-semibold" style={{ color: COLORS.ink }}>{formatSignerName(directeur, t)}</div>
              </div>
              <div className="text-center">
                <div className="text-xs font-semibold mb-8" style={{ color: COLORS.inkSoft }}>{t("teacher_in_charge_label")}</div>
                {teacher?.signature_url && <img src={teacher.signature_url} alt="" className="h-12 mx-auto object-contain mb-1" />}
                        ...
                        <td className="px-2 py-2 text-center">
                          {canEdit ? (
                            <input
                              ...
                              onBlur={(e) => { handleScoreChange(s.id, "ecrit", e.target.value); setActiveStudentId(null); }}
                          ...
                          <td className="px-2 py-2 text-center">
                            {canEdit ? (
                              <input
                                ...
                                defaultValue={g?.oral ?? ""}
                                onFocus={() => setActiveStudentId(s.id)}
                                onBlur={(e) => { handleScoreChange(s.id, "oral", e.target.value); setActiveStudentId(null); }}
                                className="w-16 px-2 py-1.5 rounded-lg text-center outline-none text-sm"
            ...
            <thead>
              <tr style={{ background: COLORS.paper }}>
                ...
                {subjects.map((subj) => (
                  <th
                    key={subj}
                    className="text-center px-3 py-3 font-semibold sticky top-0 z-10"
                    style={{
                      ...
                      <td key={subj} className="px-2 py-2 text-center">
                        {cellsReadOnly ? (
                          ...
                          <input
                            ...
                            onBlur={(e) => { handleChange(row.student.id, subj, e.target.value); setActiveCell({ studentId: null, subject: null }); }}
                            className="w-16 px-2 py-1.5 rounded-lg text-center outline-none text-sm"
