...
export default function App() {
  ...
  const loadProfile = useCallback(async (userId) => {
    const { data } = await supabase.from("profiles").select("*, schools(name, code, currency, current_year, level_type)").eq("user_id", userId).maybeSingle();
...
function MainApp({ profile, refreshProfile }) {
  ...
  const myChildren = useMemo(() => students.filter((s) => s.parent_id === profile.user_id && s.school_year === currentYear), [students, profile.user_id, currentYear]);
  ...
  const stats = useMemo(() => {
    ...
    return { nbEleves: yearStudents.length, totalDue, totalPaid, reste: totalDue - totalPaid, nbDevoirs: lessons.length, totalExpenses, solde: totalPaid - totalExpenses };
  }, [yearStudents, yearPayments, yearExpenses, lessons]);
  ...
  const saveGrade = async (studentId, subject, score, term) => {
    const { data, error } = await supabase
      .from("grades")
      .upsert({ school_id: schoolId, student_id: studentId, subject, score, term, class_name: students.find((s) => s.id === studentId)?.class_name || "" }, { onConflict: "student_id,subject,term" })
      .select();
    if (data) {
      setGrades((prev) => {
        const existing = prev.find((g) => g.student_id === studentId && g.subject === subject && g.term === term);
  ...
  const toggleRelease = async (classNameArg, term, publishedValue) => {
    const { data } = await supabase
      .from("grade_releases")
      .upsert({ school_id: schoolId, class_name: classNameArg, term, published: publishedValue }, { onConflict: "school_id,class_name,term" })
      .select();
    if (data) {
      setReleases((prev) => {
        const existing = prev.find((r) => r.class_name === classNameArg && r.term === term);
  ...
  const saveStaffPayment = async (staffUserId, month, field, value) => {
    const existing = staffPayments.find((sp) => sp.user_id === staffUserId && sp.month === month);
    const payload = {
      school_id: schoolId,
      user_id: staffUserId,
      month,
    ...
    const { data, error } = await supabase.from("staff_payments").upsert(payload, { onConflict: "user_id,month" }).select();
    if (data) {
      setStaffPayments((prev) => {
        const idx = prev.findIndex((sp) => sp.user_id === staffUserId && sp.month === month);
  ...
  const saveGradeSecondaire = async (studentId, subj, classNameArg, month, field, value) => {
    ...
    const existing = gradesSecondaire.find((g) => g.student_id === studentId && g.subject === subj && g.month === month);
    const payload = {
      school_id: schoolId, student_id: studentId, teacher_id: profile.user_id, subject: subj, class_name: classNameArg, month, term,
      ecrit: field === "ecrit" ? value : existing?.ecrit ?? null,
      oral: field === "oral" ? value : existing?.oral ?? null,
    };
    const { data, error } = await supabase.from("grades_secondaire").upsert(payload, { onConflict: "student_id,subject,month" }).select();
    if (data) setGradesSecondaire((prev) => {
      const idx = prev.findIndex((g) => g.student_id === studentId && g.subject === subj && g.month === month);
  ...
  const setAttendanceStatus = async (studentId, classNameArg, date, session, present) => {
    const { data, error } = await supabase
      .from("attendance")
      .upsert({ school_id: schoolId, student_id: studentId, class_name: classNameArg, date, session, present, recorded_by: profile.user_id }, { onConflict: "student_id,date,session" })
      .select();
    if (data) {
      setAttendance((prev) => {
        const existing = prev.find((a) => a.student_id === studentId && a.date === date && a.session === session);
  ...
  const NAV = {
    fondateur: [["dashboard", "Tableau de bord", LayoutDashboard], ["students", "Élèves", Users], ["announcements", "Annonces", Megaphone], ["attendance", "Présences", CalendarCheck], ["grades", "Notes", ClipboardList], ...(isSecondaire ? [["classteachers", "Professeurs principaux", Shield]] : []), ["lessons", "Devoirs", BookOpen], ["expenses", "Dépenses", Receipt], ["staff", "Personnel", Wallet], ["mysalary", "Mon salaire", Receipt], ["remarks", "Remarques parents", MessageSquare]],
      ...
      <main className="flex-1 p-5 md:p-10 pb-20 md:pb-10 max-w-5xl mx-auto w-full">
        {tab === "dashboard" && role === "parent" && (
          <ParentChildrenView
            ...
            payments={payments}
            onRelink={relinkSelf}
            grades={[...grades, ...[...new Set(students.map((s) => s.class_name))].flatMap((cn) => computeAnnualGrades(grades, cn))].filter((g) =>
              releases.some((r) => r.class_name === g.class_name && r.term === g.term && r.published)
            )}
        ...
        {tab === "students" && (role === "comptable" || role === "fondateur") && (
          <StudentsView students={yearStudents} paidByStudent={paidByStudent} currency={currency} readOnly={role === "fondateur" || isArchiveView} parents={parents} subscribed={profile.schools?.subscribed} studentLimit={profile.schools?.student_limit || 10} levels={levels} onAdd={() => setModal("student")} onDelete={deleteStudent} onPay={() => setModal("payment")} onLink={(s) => setLinkingStudent(s)} />
        )}
        {tab === "students" && role === "enseignant" && <StudentsReadOnlyView students={role === "enseignant" ? students.filter((s) => s.class_name === profile.class_name) : students} />}
        {tab === "lessons" && (role === "enseignant" || role === "fondateur") && (
          <LessonsTeacherView
            lessons={role === "enseignant" ? lessons.filter((l) => l.class_name === profile.class_name) : lessons}
            students={role === "enseignant" ? students.filter((s) => s.class_name === profile.class_name) : students}
            homework={homework}
        ...
        {tab === "grades" && !isSecondaire && (role === "enseignant" || role === "fondateur") && (
          <GradesView students={students} grades={grades} releases={releases} readOnly={role === "fondateur"} lockedClass={role === "enseignant" ? profile.class_name : null} onSave={saveGrade} onToggleRelease={toggleRelease} />
        )}
        {tab === "grades" && isSecondaire && (role === "enseignant" || role === "fondateur") && (
          <GradesSecondaireView
            students={students}
            gradesSecondaire={gradesSecondaire}
            releases={releases}
        ...
        {tab === "classteachers" && role === "fondateur" && (
          <ClassTeachersView students={students} teacherAssignments={teacherAssignments} classHeadTeachers={classHeadTeachers} staffProfiles={staffProfiles} onSet={setClassHeadTeacher} />
        )}
        {tab === "announcements" && (
        ...
        {tab === "staff" && (role === "comptable" || role === "fondateur") && (
          <StaffPaymentsView staffProfiles={staffProfiles} staffPayments={staffPayments} currency={currency} readOnly={role === "fondateur"} canDeactivate={role === "fondateur"} onSave={saveStaffPayment} onToggleActive={toggleStaffActive} />
        )}
        {tab === "mysalary" && role !== "parent" && (
          <MySalaryView myPayments={staffPayments.filter((sp) => sp.user_id === profile.user_id)} currency={currency} onApprove={approveStaffPayment} />
        )}
      ...
      {showNewYear && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(32,48,74,0.45)" }} onClick={() => setShowNewYear(false)}>
          <div className="w-full max-w-sm rounded-xl p-6" style={{ background: COLORS.card }} onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-sm mb-4" style={{ color: COLORS.inkSoft }}>L'année {currentYear} sera archivée en lecture seule. Tu pourras toujours la consulter et comparer.</p>
...
function StudentsView({ students, paidByStudent, currency, readOnly, parents, subscribed, studentLimit, levels, onAdd, onDelete, onPay, onLink }) {
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {classStudents.map((s, i) => {
                  ...
                  return (
                    <tr key={s.id} style={{ borderTop: i === 0 ? "none" : `1px solid ${COLORS.line}` }}>
                      <td className="px-4 py-2.5 font-medium whitespace-nowrap" style={{ color: COLORS.ink }}>{s.full_name}</td>
...
function GradesView({ students, grades, releases, readOnly, lockedClass, onSave, onToggleRelease }) {
  const classNames = useMemo(() => [...new Set(students.map((s) => s.class_name))].sort(), [students]);
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ background: COLORS.paper }}>
                  ...
                  <th className="text-center px-3 py-2.5 font-semibold" style={{ color: COLORS.inkSoft }}>Ont composé</th>
                  <th className="text-center px-3 py-2.5 font-semibold" style={{ color: COLORS.inkSoft }}>Moyenne</th>
                  ...
                  <th className="text-center px-3 py-2.5 font-semibold" style={{ color: COLORS.negative }}>% Échoués</th>
                  <th className="text-center px-3 py-2.5 font-semibold" style={{ color: COLORS.inkSoft }}>Abandons</th>
                </tr>
              </thead>
              <tbody style={{ fontFamily: "'IBM Plex Mono', monospace" }}>
                ...
                <tr style={{ borderTop: `2px solid ${COLORS.line}`, background: COLORS.paper }}>
                  ...
                  <td className="text-center px-3 py-2 font-bold"></td>
            ...
            <table className="w-full text-sm">
              ...
              <tbody style={{ fontFamily: "'IBM Plex Mono', monospace" }}>
                <tr style={{ borderTop: `1px solid ${COLORS.line}` }}>
                  <td className="px-4 py-2 font-sans font-medium" style={{ color: COLORS.ink }}>Garçons</td>
                  {subjectStats.map((s) => (
                    <td key={s.subject} className="text-center px-3 py-2">
                      <div>{s.pg}/{s.g}</div>
                      <div className="font-bold" style={{ color: COLORS.primary }}>{s.pctG}%</div>
                    </td>
                  ))}
                </tr>
                <tr style={{ borderTop: `1px solid ${COLORS.line}` }}>
                  ...
                  {subjectStats.map((s) => (
                    <td key={s.subject} className="text-center px-3 py-2">
                      <div>{s.pf}/{s.f}</div>
                      <div className="font-bold" style={{ color: COLORS.primary }}>{s.pctF}%</div>
                    </td>
                  ))}
                </tr>
                <tr style={{ borderTop: `2px solid ${COLORS.line}`, background: COLORS.paper }}>
                  ...
                  {subjectStats.map((s) => (
                    <td key={s.subject} className="text-center px-3 py-2">
                      <div className="font-bold">{s.pt}/{s.total}</div>
                      <div className="font-bold" style={{ color: COLORS.primary }}>{s.pctT}%</div>
                    </td>
                  ))}
                </tr>
              </tbody>
...
function ParentChildrenView({ children, paidByStudent, currency, payments, onRelink, grades, attendance, staffProfiles }) {
          ...
          {classStudents.map((s, i) => {
            ...
            return (
              <div key={s.id} className="flex items-center justify-between px-5 py-3" style={{ borderTop: i === 0 ? "none" : `1px solid ${COLORS.line}` }}>
                <span className="font-medium" style={{ color: COLORS.ink }}>{s.full_name}</span>
                {readOnly ? (
                  status ? (
