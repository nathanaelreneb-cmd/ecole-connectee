...
function MainApp({ profile, refreshProfile }) {
  ...
  const [tab, setTab] = useState(role === "comptable" ? "students" : role === "enseignant" ? "grades" : role === "personnel" ? "mysalary" : role === "directeur" ? "announcements" : "dashboard");
  ...
  const uploadAvatar = async (file) => {
    ...
    if (upErr) { alert("Erreur : " + upErr.message); return null; }
  ...
  const uploadSignature = async (file) => {
    ...
    const url = `${data.publicUrl}?t=${Date.now()}`;
    await supabase.from("profiles").update({ signature_url: url }).eq("user_id", profile.user_id);
    await refreshProfile();
    return url;
  };
  ...
  return (
    <div className="min-h-screen w-full flex" style={{ background: COLORS.paper, fontFamily: "'Inter', sans-serif" }}>
      ...
      <main className="flex-1 p-5 md:p-10 pb-20 md:pb-10 max-w-5xl mx-auto w-full">
        ...
        {tab === "announcements" && (
          <AnnouncementsView announcements={announcements} canWrite={role === "fondateur" || role === "directeur"} onAdd={() => setModal("announcement")} onDelete={deleteAnnouncement} />
        )}
