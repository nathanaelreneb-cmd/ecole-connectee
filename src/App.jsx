...
function MainApp({ profile, refreshProfile }) {
  ...
  const uploadAvatar = async (file) => {
    try {
      const blob = await compressImageFile(file, 500, 0.8);
      if (!blob) { alert("Impossible de lire cette image. Réessaie avec une autre photo."); return null; }
      const path = `${profile.user_id}/avatar.jpg`;
      const { error: upErr } = await supabase.storage.from("avatars").upload(path, blob, { upsert: true, contentType: "image/jpeg" });
      if (upErr) { alert("Erreur d'envoi : " + upErr.message); return null; }
      const { data } = supabase.storage.from("avatars").getPublicUrl(path);
      const url = `${data.publicUrl}?t=${Date.now()}`;
      const { error: dbErr } = await supabase.from("profiles").update({ avatar_url: url }).eq("user_id", profile.user_id);
      if (dbErr) { alert("Erreur d'enregistrement : " + dbErr.message); return null; }
      await refreshProfile();
      return url;
    } catch (e) {
      alert("Erreur : " + (e.message || "connexion interrompue, réessaie."));
      return null;
    }
  };
  ...
  const uploadSignature = async (file) => {
    try {
      const blob = await compressImageFile(file, 500, 0.85);
      if (!blob) { alert("Impossible de lire cette image. Réessaie avec une autre photo."); return null; }
      const path = `${profile.user_id}/signature.jpg`;
      const { error: upErr } = await supabase.storage.from("signatures").upload(path, blob, { upsert: true, contentType: "image/jpeg" });
      if (upErr) { alert("Erreur d'envoi : " + upErr.message); return null; }
  ...
  const NAV = {
    ...
    parent: [["dashboard", t("nav_my_children"), GraduationCap], ["revision", t("nav_revision"), BookOpen], ["lessons", t("nav_lessons"), BookOpen], ["announcements", t("nav_announcements"), Megaphone], ["remarks", t("nav_my_remarks"), MessageSquare], ["pay", t("nav_pay_remotely"), Smartphone]],
    ...
    <Modal title={t("my_profile")} onClose={onClose}>
      <div className="space-y-5">
        <div className="flex flex-col items-center">
          ...
          <button onClick={() => avatarInputRef.current?.click()} className="text-xs font-semibold mt-2" style={{ color: COLORS.primary }}>{t("change_photo")}</button>
        </div>
        ...
        {canSign && (
          <div>
            ...
            <input ref={signatureInputRef} type="file" accept="image/*" className="hidden" onChange={(e) => pickSignature(e.target.files?.[0])} />
...
function ExerciseHelpTab({ lang }) {
  ...
  const handleFile = (fileObj) => {
    ...
    // Images : on redimensionne et compresse côté téléphone avant l'envoi
    // (une photo de téléphone peut peser 3-8 Mo, trop lourd pour le serveur).
    const reader = new FileReader();
    reader.onload = () => {
      const img = new Image();
      img.onload = () => {
        ...
        setFile({ base64, mimeType: "image/jpeg", previewUrl: compressedDataUrl, name: fileObj.name });
      };
      img.onerror = () => setError(t("exercise_bad_format"));
      img.src = reader.result;
    };
    reader.readAsDataURL(fileObj);
  };
