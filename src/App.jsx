...
function MainApp({ profile, refreshProfile }) {
  ...
  const loadAll = useCallback(async () => {
    try {
      const [s, p, l, h, a, par, exp, gr, rel, staff, sp, rem, att, ta, cht, subj, grs, tf, svc, svcSub, svcPay] = await Promise.all([
      ...
      const allSucceeded = [s, p, l, h, a, par, exp, gr, rel, staff, sp, rem, att, ta, cht, subj, grs, tf, svc, svcSub, svcPay].every((r) => r.data);
      if (allSucceeded) {
        const snapshot = {
          ...
          services: svc.data, serviceSubscriptions: svcSub.data, servicePayments: svcPay.data,
        };
  ...
  const { t, lang } = useLang();
  const NAV = {
    fondateur: [["dashboard", t("nav_dashboard"), LayoutDashboard], ["students", t("nav_students"), Users], ["tuitionfees", t("nav_tuition_fees"), Banknote], ["services", t("nav_services"), Receipt], ["attendance", t("nav_attendance"), CalendarCheck], ["grades", t("nav_grades"), ClipboardList], ...(isSecondaire ? [["classteachers", t("nav_head_teachers"), Shield]] : [["classassign", t("nav_class_assign"), Shield]]), ["expenses", t("nav_expenses"), Receipt], ["staff", t("nav_staff"), Wallet], ["mysalary", t("nav_my_salary"), Banknote]],
    comptable: [["students", t("nav_students_payments"), Users], ["services", t("nav_services"), Receipt], ["dashboard", t("nav_stats"), LayoutDashboard], ["expenses", t("nav_expenses"), Receipt], ["staff", t("nav_staff"), Wallet], ["mysalary", t("nav_my_salary"), Banknote]],
...
function ServicesView({ role, students, services, serviceSubscriptions, servicePayments, currency, onSaveService, onDeleteService, onEnroll, onUnenroll, onAddPayment }) {
  ...
  return (
    <div>
      ...
      <p className="mb-5" style={{ color: COLORS.inkSoft }}>{t("services_subtitle")}</p>
      ...
      {services.length === 0 ? (
        ...
        <>
          ...
          <input
            ...
            placeholder={t("search_student_placeholder")}
            className="w-full px-4 py-3 rounded-xl outline-none text-sm mb-5"
            style={inputStyle}
          />
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ background: COLORS.paper }}>
                ...
                <th className="text-left px-4 py-3 font-semibold whitespace-nowrap sticky left-0 top-0 z-20" style={{ color: COLORS.inkSoft, background: COLORS.paper }}>{t("student")}</th>
                {subjects.map((subj) => (
                  <th key={subj} className="text-center px-3 py-3 font-semibold whitespace-nowrap sticky top-0 z-10" style={{ color: COLORS.inkSoft, background: COLORS.paper }}>{subj}</th>
                ))}
