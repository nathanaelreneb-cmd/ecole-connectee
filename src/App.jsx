...
function MainApp({ profile, refreshProfile }) {
  ...
  const assignTeacherFondateur = async (teacherId, subj, classNameArg) => {
    const { data, error } = await supabase.from("teacher_assignments").insert({ school_id: schoolId, teacher_id: teacherId, subject: subj, class_name: classNameArg }).select();
...
async function exportStyledExcel({ filename, title, headers, rows, boldRows = [] }) {
  ...
  rows.forEach((r, i) => {
    ...
    row.eachCell((cell) => {
      ...
      if (r.isRed) cell.font = { color: { argb: "FFB23B32" } };
...
function GradesSecondaireView({ students, gradesSecondaire, releases, allAssignments, myUserId, classHeadTeachers, subjectsSecondaire, currency, readOnly, isHeadTeacherOf, onSave, onSetCoefficient, onToggleRelease }) {
  ...
  const myOwnAssignments = useMemo(() => allAssignments.filter((a) => a.teacher_id === myUserId), [allAssignments, myUserId]);
  ...
  const renderSaisie = () => {
    ...
    const exportSaisieExcel = () => {
      ...
      classStudents.forEach((s) => {
        ...
        if (isComp) rows.push([s.full_name, g?.ecrit ?? "", trimAvg(s.id) !== null ? trimAvg(s.id).toFixed(2) : ""]);
        else rows.push([s.full_name, g?.ecrit ?? "", g?.oral ?? "", monthAvg(s.id) !== null ? monthAvg(s.id).toFixed(2) : "", trimAvg(s.id) !== null ? trimAvg(s.id).toFixed(2) : ""]);
      });
      const csv = rows.map((r) => r.map((v) => `"${String(v).replace(/"/g, '""')}"`).join(",")).join("\n");
      const blob = new Blob(["\uFEFF" + csv], { type: "text/csv;charset=utf-8;" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `notes_${className}_${subject}_${month}_${new Date().toISOString().slice(0, 10)}.csv`;
      a.click();
      URL.revokeObjectURL(url);
    };
  ...
  const renderBulletin = () => {
    ...
    const exportBulletinExcel = () => {
      const rowsCsv = [[t("rank"), t("student"), ...allSubjects, t("general_average")]];
      rows.forEach((row, i) => {
        rowsCsv.push([i + 1, row.student.full_name, ...row.subjectAvgs.map((sa) => (sa.avg !== null ? sa.avg.toFixed(2) : "")), row.moyenneGenerale !== null ? row.moyenneGenerale.toFixed(2) : ""]);
      });
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `bulletin_${className}_${term}_${new Date().toISOString().slice(0, 10)}.csv`;
      a.click();
      URL.revokeObjectURL(url);
    };
...
function ClassTeachersView({ students, teacherAssignments, classHeadTeachers, staffProfiles, existingClassNames, existingSubjectNames, onSet, onAssign, onUnassign }) {
  const { t } = useLang();
  const [saving, setSaving] = useState(null);
                ...
                {teacherAssignments.map((a) => {
                  ...
                  return (
                    <div key={a.id} className="flex items-center justify-between px-3 py-2 rounded-lg text-sm" style={{ background: COLORS.paper }}>
                      <span style={{ color: COLORS.ink }}>{tch?.full_name || "—"} <span style={{ color: COLORS.inkSoft }}>· {a.subject} · {a.class_name}</span></span>
...
function GradesView({ students, grades, releases, readOnly, lockedClass, passThreshold, onSave, onToggleRelease }) {
  ...
  const exportExcel = async () => {
    ...
    const styleDataRow = (row, isRed) => row.eachCell((c) => { c.border = thinBorder; c.alignment = { horizontal: "center", vertical: "middle" }; if (isRed) c.font = { color: { argb: "FFB23B32" } }; });
    ...
    rows.forEach((r) => {
      ...
      styleDataRow(row, r.student.gender === "Fille");
      row.getCell(1).alignment = { horizontal: "left", vertical: "middle" };
    });

    sheet.addRow([]);
    const statsTitle = sheet.addRow(["Tableau statistique"]);
    statsTitle.getCell(1).font = { bold: true, size: 12, color: { argb: "FF20304A" } };
    styleHeaderRow(sheet.addRow(["Statistiques", "Garçons", "Filles", "Total"]));
    styleDataRow(sheet.addRow(["Inscrits", summaryStats.inscrits.garcons, summaryStats.inscrits.filles, summaryStats.inscrits.total]));
    styleDataRow(sheet.addRow(["Ont composé", summaryStats.composes.garcons, summaryStats.composes.filles, summaryStats.composes.total]));
    styleDataRow(sheet.addRow(["Moyenne", summaryStats.moyenneG?.toFixed(2) ?? "—", summaryStats.moyenneF?.toFixed(2) ?? "—", ""]));
    styleDataRow(sheet.addRow(["Admis", summaryStats.admis.garcons, summaryStats.admis.filles, summaryStats.admis.total]));
    styleDataRow(sheet.addRow(["% Admis", summaryStats.admisPctG, summaryStats.admisPctF, summaryStats.admisPctT]));
    styleDataRow(sheet.addRow(["Échoués", summaryStats.echoues.garcons, summaryStats.echoues.filles, summaryStats.echoues.total]));
    styleDataRow(sheet.addRow(["% Échoués", summaryStats.echouePctG, summaryStats.echouePctF, summaryStats.echouePctT]));
    styleDataRow(sheet.addRow(["Abandonnés", summaryStats.abandons.garcons, summaryStats.abandons.filles, summaryStats.abandons.total]));

    sheet.addRow([]);
    const subjTitle = sheet.addRow(["Résultats par matière"]);
    subjTitle.getCell(1).font = { bold: true, size: 12, color: { argb: "FF20304A" } };
    styleHeaderRow(sheet.addRow(["Matière", "Garçons ayant la moyenne", "Filles ayant la moyenne", "% Garçons", "% Filles", "% Total"]));
    subjectStats.forEach((s) => styleDataRow(sheet.addRow([s.subject, `${s.pg}/${s.g}`, `${s.pf}/${s.f}`, s.pctG, s.pctF, s.pctT])));
        ...
        <>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ background: COLORS.ink }}>
                  <th className="text-left px-4 py-2.5 font-semibold" style={{ color: "#fff" }}>{t("statistical_table")}</th>
                  <th className="text-center px-3 py-2.5 font-semibold" style={{ color: "#fff" }}>{t("boys")}</th>
                  <th className="text-center px-3 py-2.5 font-semibold" style={{ color: "#FFD1CC" }}>{t("girls")}</th>
                  <th className="text-center px-3 py-2.5 font-semibold" style={{ color: "#fff" }}>{t("total")}</th>
                </tr>
              </thead>
              <tbody style={{ fontFamily: "'IBM Plex Mono', monospace" }}>
                <tr style={{ borderTop: `1px solid ${COLORS.line}` }}>
                  <td className="px-4 py-2 font-sans font-medium" style={{ color: COLORS.ink }}>{t("enrolled")}</td>
                ...
                <tr style={{ borderTop: `1px solid ${COLORS.line}`, background: COLORS.paper }}>
                  <td className="px-4 py-2 font-sans font-medium" style={{ color: COLORS.ink }}>{t("composed")}</td>
                ...
                <tr style={{ borderTop: `1px solid ${COLORS.line}` }}>
                  <td className="px-4 py-2 font-sans font-medium" style={{ color: COLORS.ink }}>{t("average")}</td>
                  <td className="text-center px-3 py-2">{summaryStats.moyenneG?.toFixed(2) ?? "—"}</td>
                  <td className="text-center px-3 py-2" style={{ color: "#B23B32" }}>{summaryStats.moyenneF?.toFixed(2) ?? "—"}</td>
                  <td className="text-center px-3 py-2"></td>
                </tr>
                <tr style={{ borderTop: `1px solid ${COLORS.line}`, background: COLORS.paper }}>
                  <td className="px-4 py-2 font-sans font-medium" style={{ color: COLORS.ink }}>{t("admitted")}</td>
                  <td className="text-center px-3 py-2" style={{ color: COLORS.positive }}>{summaryStats.admis.garcons}</td>
                  <td className="text-center px-3 py-2" style={{ color: "#B23B32" }}>{summaryStats.admis.filles}</td>
                  <td className="text-center px-3 py-2 font-bold" style={{ color: COLORS.positive }}>{summaryStats.admis.total}</td>
                </tr>
                <tr style={{ borderTop: `1px solid ${COLORS.line}` }}>
                  <td className="px-4 py-2 font-sans font-medium" style={{ color: COLORS.ink }}>{t("pct_admitted")}</td>
                  <td className="text-center px-3 py-2" style={{ color: COLORS.positive }}>{summaryStats.admisPctG}%</td>
                  <td className="text-center px-3 py-2" style={{ color: "#B23B32" }}>{summaryStats.admisPctF}%</td>
                  <td className="text-center px-3 py-2 font-bold" style={{ color: COLORS.positive }}>{summaryStats.admisPctT}%</td>
                </tr>
                <tr style={{ borderTop: `1px solid ${COLORS.line}`, background: COLORS.paper }}>
                  <td className="px-4 py-2 font-sans font-medium" style={{ color: COLORS.ink }}>{t("failed")}</td>
                  <td className="text-center px-3 py-2" style={{ color: COLORS.negative }}>{summaryStats.echoues.garcons}</td>
                  <td className="text-center px-3 py-2" style={{ color: "#B23B32" }}>{summaryStats.echoues.filles}</td>
                  <td className="text-center px-3 py-2 font-bold" style={{ color: COLORS.negative }}>{summaryStats.echoues.total}</td>
                </tr>
                <tr style={{ borderTop: `1px solid ${COLORS.line}` }}>
                  <td className="px-4 py-2 font-sans font-medium" style={{ color: COLORS.ink }}>{t("pct_failed")}</td>
                  <td className="text-center px-3 py-2" style={{ color: COLORS.negative }}>{summaryStats.echouePctG}%</td>
                  <td className="text-center px-3 py-2" style={{ color: "#B23B32" }}>{summaryStats.echouePctF}%</td>
                  <td className="text-center px-3 py-2 font-bold" style={{ color: COLORS.negative }}>{summaryStats.echouePctT}%</td>
                </tr>
                <tr style={{ borderTop: `2px solid ${COLORS.line}`, background: COLORS.paper }}>
                  <td className="px-4 py-2 font-sans font-bold" style={{ color: COLORS.ink }}>{t("dropped_out")}</td>
                  <td className="text-center px-3 py-2 font-bold">{summaryStats.abandons.garcons}</td>
                  <td className="text-center px-3 py-2 font-bold" style={{ color: "#B23B32" }}>{summaryStats.abandons.filles}</td>
          ...
          <div className="rounded-2xl overflow-x-auto" style={{ background: COLORS.card, border: `1px solid ${COLORS.line}` }}>
            <table className="w-full text-sm">
              <thead>
                <tr style={{ background: COLORS.ink }}>
                  <th className="text-left px-4 py-2.5 font-semibold whitespace-nowrap" style={{ color: "#fff" }}>{t("subject_col")}</th>
                  <th className="text-center px-3 py-2.5 font-semibold" style={{ color: "#fff" }}>{t("boys")}</th>
                  <th className="text-center px-3 py-2.5 font-semibold" style={{ color: "#FFD1CC" }}>{t("girls")}</th>
                  <th className="text-center px-3 py-2.5 font-semibold" style={{ color: "#fff" }}>{t("pct_admitted")} G</th>
                  <th className="text-center px-3 py-2.5 font-semibold" style={{ color: "#FFD1CC" }}>{t("pct_admitted")} F</th>
                  <th className="text-center px-3 py-2.5 font-semibold" style={{ color: "#fff" }}>{t("pct_admitted")} T</th>
                </tr>
              </thead>
              <tbody style={{ fontFamily: "'IBM Plex Mono', monospace" }}>
                {subjectStats.map((s, i) => (
                  <tr key={s.subject} style={{ borderTop: `1px solid ${COLORS.line}`, background: i % 2 === 1 ? COLORS.paper : "transparent" }}>
                    <td className="px-4 py-2.5 font-sans font-medium whitespace-nowrap" style={{ color: COLORS.ink }}>{s.subject}</td>
                    <td className="text-center px-3 py-2.5">{s.pg}/{s.g}</td>
                    <td className="text-center px-3 py-2.5" style={{ color: "#B23B32" }}>{s.pf}/{s.f}</td>
                    <td className="text-center px-3 py-2.5 font-bold" style={{ color: COLORS.primary }}>{s.pctG}%</td>
                    <td className="text-center px-3 py-2.5 font-bold" style={{ color: "#B23B32" }}>{s.pctF}%</td>
                    <td className="text-center px-3 py-2.5 font-bold" style={{ color: COLORS.primary }}>{s.pctT}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
