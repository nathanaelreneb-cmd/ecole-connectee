        ...
        <div className="space-y-4">
          ...
          {role === "enseignant" && (
            <div>
              ...
              <p className="text-xs mt-1.5" style={{ color: COLORS.inkSoft }}>Tu ne pourras gérer devoirs et notes que pour cette classe — utilise exactement le même nom que le comptable pour cette classe.</p>
            </div>
          )}
...
export default function App() {
  ...
  const loadProfile = useCallback(async (userId) => {
    const { data } = await supabase.from("profiles").select("*, schools(name, code, currency, current_year)").eq("user_id", userId).maybeSingle();
...
function MainApp({ profile, refreshProfile }) {
  ...
  const loadAll = useCallback(async () => {
    const [s, p, l, h, a, par, exp, gr, rel, staff, sp, rem, att] = await Promise.all([
      ...
      supabase.from("profiles").select("user_id, full_name, role, class_name, fonction, phone").neq("role", "parent"),
  ...
  const startNewYear = async (newYear) => {
    ...
    if (!error) {
      profile.schools.current_year = newYear;
      setViewYear(newYear);
    }
    return error;
  };
  ...
  const NAV = {
    ...
    enseignant: [["lessons", "Devoirs", BookOpen], ["grades", "Notes", ClipboardList], ["attendance", "Présences", CalendarCheck], ["students", "Élèves", Users], ["mysalary", "Mon salaire", Receipt]],
    parent: [["dashboard", "Mes enfants", GraduationCap], ["lessons", "Devoirs", BookOpen], ["announcements", "Annonces", Megaphone], ["remarks", "Mes remarques", MessageSquare]],
    ...
    <div className="min-h-screen w-full flex" style={{ background: COLORS.paper, fontFamily: "'Inter', sans-serif" }}>
      ...
      <main className="flex-1 p-5 md:p-10 pb-20 md:pb-10 max-w-5xl mx-auto w-full">
        ...
        {tab === "students" && (role === "comptable" || role === "fondateur") && (
          <StudentsView students={yearStudents} paidByStudent={paidByStudent} currency={currency} readOnly={role === "fondateur" || isArchiveView} parents={parents} subscribed={profile.schools?.subscribed} studentLimit={profile.schools?.student_limit || 10} onAdd={() => setModal("student")} onDelete={deleteStudent} onPay={() => setModal("payment")} onLink={(s) => setLinkingStudent(s)} />
        )}
        ...
        {tab === "staff" && (role === "comptable" || role === "fondateur") && (
          <StaffPaymentsView staffProfiles={staffProfiles} staffPayments={staffPayments} currency={currency} readOnly={role === "fondateur"} onSave={saveStaffPayment} />
        )}
...
function StaffPaymentsView({ staffProfiles, staffPayments, currency, readOnly, onSave }) {
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {staffProfiles.map((p, i) => {
                ...
                return (
                  <tr key={p.user_id} style={{ borderTop: i === 0 ? "none" : `1px solid ${COLORS.line}` }}>
