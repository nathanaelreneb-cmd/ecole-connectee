...
import {
  ...
  Wallet, TrendingUp, AlertTriangle, BookOpen, FileSpreadsheet, Smartphone, TrendingDown, Receipt, Shield, ClipboardList, ArrowDownWideNarrow, MessageSquare, Menu, CalendarCheck, Languages, Share2, Banknote, Contact, BookMarked, QrCode, Eye, EyeOff, Sparkles, Package, Coins, Download, Printer, ArrowLeft,
} from "lucide-react";
...
function GradesView({ students, grades, releases, readOnly, canPublish, lockedClass, passThreshold, staffProfiles, onSave, onToggleRelease, onPublishAll, schoolName, bulletinAppreciations, onSaveAppreciation }) {
  ...
  useEffect(() => {
    // Sur téléphone, le clavier prend souvent la moitié de l'écran : on rapproche la ligne
    // active du haut du tableau pour qu'elle reste visible juste sous l'en-tête des matières,
    // au lieu de se retrouver cachée derrière le clavier.
    // On mesure les positions réelles à l'écran (pas via offsetTop, peu fiable dans un tableau
    // imbriqué) et on laisse le clavier du téléphone le temps de s'ouvrir avant de calculer.
    if (!activeCell.studentId || !tableScrollRef.current) return;
    const timer = setTimeout(() => {
      const container = tableScrollRef.current;
      if (!container) return;
      const rowEl = document.getElementById(`row-${activeCell.studentId}`);
      const headerEl = container.querySelector("thead");
      if (!rowEl || !headerEl) return;
      const containerRect = container.getBoundingClientRect();
      const rowRect = rowEl.getBoundingClientRect();
      const headerHeight = headerEl.getBoundingClientRect().height;
      const desiredTop = containerRect.top + headerHeight + 4;
