...
import {
  ...
  Wallet, TrendingUp, AlertTriangle, BookOpen, FileSpreadsheet, Smartphone, TrendingDown, Receipt, Shield, ClipboardList, ArrowDownWideNarrow, MessageSquare, Menu, CalendarCheck, Languages, Share2, Banknote, Contact, BookMarked, QrCode, Eye, EyeOff, Sparkles,
} from "lucide-react";
...
const TRANSLATIONS = {
  fr: {
    ...
    signature_hint: "Photo de ta signature manuscrite sur fond clair — elle apparaîtra sur les bulletins que tu signes.",
  ...
  en: {
    ...
    signature_hint: "A photo of your handwritten signature on a light background — it will appear on report cards you sign.",
...
function MainApp({ profile, refreshProfile }) {
  ...
  const loadAll = useCallback(async () => {
    try {
      const [s, p, l, h, a, par, exp, gr, rel, staff, sp, rem, att, ta, cht, subj, grs, tf] = await Promise.all([
      ...
      const allSucceeded = [s, p, l, h, a, par, exp, gr, rel, staff, sp, rem, att, ta, cht, subj, grs, tf].every((r) => r.data);
      if (allSucceeded) {
        const snapshot = {
    ...
    for (const action of queue) {
      try {
        ...
        if (error) {
          ...
          if (action.localId && ["students", "payments", "expenses", "announcements"].includes(action.table)) {
        ...
        if (!data) { remaining.push(action); continue; }
  ...
  const { t, lang } = useLang();
  const NAV = {
    fondateur: [["dashboard", t("nav_dashboard"), LayoutDashboard], ["students", t("nav_students"), Users], ["tuitionfees", t("nav_tuition_fees"), Banknote], ["attendance", t("nav_attendance"), CalendarCheck], ["grades", t("nav_grades"), ClipboardList], ...(isSecondaire ? [["classteachers", t("nav_head_teachers"), Shield]] : [["classassign", t("nav_class_assign"), Shield]]), ["expenses", t("nav_expenses"), Receipt], ["staff", t("nav_staff"), Wallet], ["mysalary", t("nav_my_salary"), Banknote]],
      ...
      <main className="flex-1 p-5 md:p-10 pb-20 md:pb-10 max-w-5xl mx-auto w-full">
        ...
        {tab === "grades" && !isSecondaire && (role === "enseignant" || role === "fondateur" || role === "directeur") && (
          <>
            ...
            <GradesView students={yearStudents} grades={yearGrades} releases={yearReleases} readOnly={role === "fondateur" || role === "directeur"} canPublish={role === "directeur"} lockedClass={role === "enseignant" ? profile.class_name : null} passThreshold={isSecondaire ? 10 : 5} staffProfiles={staffProfiles} onSave={saveGrade} onToggleRelease={toggleRelease} onPublishAll={publishAllReleases} />
          </>
        )}
        {tab === "grades" && isSecondaire && (role === "enseignant" || role === "fondateur" || role === "directeur") && (
          <>
        ...
        {tab === "classteachers" && (role === "fondateur" || role === "directeur") && (
        ...
        {tab === "classassign" && (role === "fondateur" || role === "directeur") && (
          ...
          {filtered.map((s, i) => {
            ...
            return (
              <div key={s.id} style={{ borderTop: i === 0 ? "none" : `1px solid ${COLORS.line}`, opacity: s.hidden ? 0.55 : 1 }}>
                ...
                >
                  <div className="min-w-0">
                    <div className="font-semibold truncate flex items-center gap-1.5" style={{ color: COLORS.ink }}>
                      {s.name}
                      {s.hidden && <EyeOff size={13} style={{ color: COLORS.inkSoft }} />}
                    </div>
...
function GradesSecondaireView({ students, gradesSecondaire, releases, allAssignments, myUserId, classHeadTeachers, subjectsSecondaire, currency, readOnly, canPublish, staffProfiles, isHeadTeacherOf, onSave, onSetCoefficient, onToggleRelease, onPublishAll }) {
  const { t } = useLang();
        ...
        {canEdit && (
          <div className="rounded-xl px-4 py-3 mb-5 flex items-center justify-between flex-wrap gap-3" style={{ background: COLORS.primarySoft }}>
            ...
            <div className="flex gap-2">
              <input type="text" inputMode="decimal" pattern="[0-9]*[.,]?[0-9]*" value={coefInput} onChange={(e) => setCoefInput(e.target.value)} placeholder={t("new_coef")} className="w-24 px-2 py-1.5 rounded-lg text-sm outline-none" style={inputStyle} />
                    ...
                    <tr key={s.id} style={{ borderTop: i === 0 ? "none" : `1px solid ${COLORS.line}` }}>
                      ...
                      {isComp ? (
                        <td className="px-2 py-2 text-center">
                          {canEdit ? (
                            <input key={`${subject}-${month}-comp`} type="text" inputMode="decimal" pattern="[0-9]*[.,]?[0-9]*" defaultValue={g?.ecrit ?? ""} onBlur={(e) => handleScoreChange(s.id, "ecrit", e.target.value)} className="w-16 px-2 py-1.5 rounded-lg text-center outline-none text-sm" style={{ ...inputStyle, opacity: saving === `${s.id}-ecrit` ? 0.5 : 1 }} />
                          ) : (
                        ...
                        <>
                          <td className="px-2 py-2 text-center">
                            {canEdit ? (
                              <input key={`${subject}-${month}-ecrit`} type="text" inputMode="decimal" pattern="[0-9]*[.,]?[0-9]*" defaultValue={g?.ecrit ?? ""} onBlur={(e) => handleScoreChange(s.id, "ecrit", e.target.value)} className="w-16 px-2 py-1.5 rounded-lg text-center outline-none text-sm" style={{ ...inputStyle, opacity: saving === `${s.id}-ecrit` ? 0.5 : 1 }} />
                            ) : (
                              <span style={{ fontFamily: "'IBM Plex Mono', monospace", color: COLORS.ink }}>{g?.ecrit ?? "—"}</span>
                            )}
                          </td>
                          <td className="px-2 py-2 text-center">
                            {canEdit ? (
                              <input key={`${subject}-${month}-oral`} type="text" inputMode="decimal" pattern="[0-9]*[.,]?[0-9]*" defaultValue={g?.oral ?? ""} onBlur={(e) => handleScoreChange(s.id, "oral", e.target.value)} className="w-16 px-2 py-1.5 rounded-lg text-center outline-none text-sm" style={{ ...inputStyle, opacity: saving === `${s.id}-oral` ? 0.5 : 1 }} />
                            ) : (
    ...
    return (
      <>
        <div className="flex justify-end gap-2 mb-3 flex-wrap">
          ...
          <button onClick={shareBulletinExcel} disabled={sharingBulletin} className="flex items-center gap-1.5 px-4 py-2.5 rounded-lg font-medium text-sm" style={{ background: COLORS.ink, color: "#fff" }}>
            {sharingBulletin ? <Loader2 className="animate-spin" size={16} /> : <Share2 size={16} />} {t("share_button")}
          </button>
          {canPublish && (
            <>
              ...
              <button onClick={shareAllBulletinExcel} disabled={sharingAllBulletin} className="flex items-center gap-1.5 px-4 py-2.5 rounded-lg font-medium text-sm" style={{ background: COLORS.ink, color: "#fff" }}>
                {sharingAllBulletin ? <Loader2 className="animate-spin" size={16} /> : <Share2 size={16} />} {t("share_all_classes")}
              </button>
            </>
          )}
        </div>
...
function GradesView({ students, grades, releases, readOnly, canPublish, lockedClass, passThreshold, staffProfiles, onSave, onToggleRelease, onPublishAll }) {
  const { t } = useLang();
    ...
    <div>
      <div className="flex items-center justify-between mb-1 flex-wrap gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={shareExcel} disabled={sharingOne} className="flex items-center gap-1.5 px-4 py-2.5 rounded-lg font-medium text-sm" style={{ background: COLORS.ink, color: "#fff" }}>
            {sharingOne ? <Loader2 className="animate-spin" size={16} /> : <Share2 size={16} />} {t("share_button")}
          </button>
          {canPublish && (
            <>
              ...
              <button onClick={shareAllExcel} disabled={sharingAll} className="flex items-center gap-1.5 px-4 py-2.5 rounded-lg font-medium text-sm" style={{ background: COLORS.ink, color: "#fff" }}>
                {sharingAll ? <Loader2 className="animate-spin" size={16} /> : <Share2 size={16} />} {t("share_all_classes")}
              </button>
            </>
          )}
          <button
    ...
    <Modal title={isEdit ? t("edit_student") : t("new_student")} onClose={onClose}>
      <div className="space-y-4">
        ...
        <div>
          ...
          {role === "fondateur" ? (
            <>
              <input type="text" inputMode="decimal" pattern="[0-9]*[.,]?[0-9]*" value={totalDue} onChange={(e) => { setTotalDue(e.target.value); setTotalDueTouched(true); }} className="w-full px-3 py-2.5 rounded-lg outline-none" style={inputStyle} />
            ...
            <>
              <input type="text" inputMode="decimal" pattern="[0-9]*[.,]?[0-9]*" value={totalDue} disabled className="w-full px-3 py-2.5 rounded-lg outline-none font-semibold" style={{ ...inputStyle, background: "#FFF3D6", borderColor: "#E8B84B", color: COLORS.ink, opacity: 1 }} />
  ...
  return (
    <Modal title={t("new_expense")} onClose={onClose}>
      <div className="space-y-4">
        ...
        <div><FieldLabel>{t("amount_label")}</FieldLabel><input type="text" inputMode="decimal" pattern="[0-9]*[.,]?[0-9]*" value={amount} onChange={(e) => setAmount(e.target.value)} className="w-full px-3 py-2.5 rounded-lg outline-none" style={inputStyle} /></div>
  ...
  return (
    <Modal title={t("record_payment")} onClose={onClose}>
      <div className="space-y-4">
        ...
        <div><FieldLabel>{t("amount_currency")} ({currency})</FieldLabel><input type="text" inputMode="decimal" pattern="[0-9]*[.,]?[0-9]*" value={amount} onChange={(e) => setAmount(e.target.value)} className="w-full px-3 py-2.5 rounded-lg outline-none" style={inputStyle} /></div>
        <div><FieldLabel>{t("date_label")}</FieldLabel><input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full px-3 py-2.5 rounded-lg outline-none" style={inputStyle} /></div>
        <button onClick={submit} className="w-full py-3 rounded-lg font-semibold mt-2" style={{ background: COLORS.ink, color: "#fff" }}>{t("save")}</button>
      </div>
    </Modal>
      ...
      <div className="rounded-2xl p-5 mb-6" style={{ background: COLORS.card, border: `1px solid ${COLORS.line}` }}>
        ...
        <div className="grid sm:grid-cols-2 gap-3 mb-4">
          ...
          <div>
            ...
            <input type="text" inputMode="decimal" pattern="[0-9]*[.,]?[0-9]*" min={4} max={20} value={nbQuestions} onChange={(e) => setNbQuestions(e.target.value)} className="w-full px-3 py-2.5 rounded-lg outline-none text-sm" style={inputStyle} />
          </div>
        </div>
