        ...
        {canEdit && (
          <div className="rounded-xl px-4 py-3 mb-5 flex items-center justify-between flex-wrap gap-3" style={{ background: COLORS.primarySoft }}>
            ...
            <div className="flex gap-2">
              <input type="number" inputMode="decimal" value={coefInput} onChange={(e) => setCoefInput(e.target.value)} placeholder={t("new_coef")} className="w-24 px-2 py-1.5 rounded-lg text-sm outline-none" style={inputStyle} />
                    ...
                    <tr key={s.id} style={{ borderTop: i === 0 ? "none" : `1px solid ${COLORS.line}` }}>
                      ...
                      {isComp ? (
                        <td className="px-2 py-2 text-center">
                          {canEdit ? (
                            <input key={`${subject}-${month}-comp`} type="number" inputMode="decimal" defaultValue={g?.ecrit ?? ""} onBlur={(e) => handleScoreChange(s.id, "ecrit", e.target.value)} className="w-16 px-2 py-1.5 rounded-lg text-center outline-none text-sm" style={{ ...inputStyle, opacity: saving === `${s.id}-ecrit` ? 0.5 : 1 }} />
                          ) : (
                        ...
                        <>
                          <td className="px-2 py-2 text-center">
                            {canEdit ? (
                              <input key={`${subject}-${month}-ecrit`} type="number" inputMode="decimal" defaultValue={g?.ecrit ?? ""} onBlur={(e) => handleScoreChange(s.id, "ecrit", e.target.value)} className="w-16 px-2 py-1.5 rounded-lg text-center outline-none text-sm" style={{ ...inputStyle, opacity: saving === `${s.id}-ecrit` ? 0.5 : 1 }} />
                            ) : (
                              <span style={{ fontFamily: "'IBM Plex Mono', monospace", color: COLORS.ink }}>{g?.ecrit ?? "—"}</span>
                            )}
                          </td>
                          <td className="px-2 py-2 text-center">
                            {canEdit ? (
                              <input key={`${subject}-${month}-oral`} type="number" inputMode="decimal" defaultValue={g?.oral ?? ""} onBlur={(e) => handleScoreChange(s.id, "oral", e.target.value)} className="w-16 px-2 py-1.5 rounded-lg text-center outline-none text-sm" style={{ ...inputStyle, opacity: saving === `${s.id}-oral` ? 0.5 : 1 }} />
                            ) : (
    ...
    <Modal title={isEdit ? t("edit_student") : t("new_student")} onClose={onClose}>
      <div className="space-y-4">
        ...
        <div>
          ...
          {role === "fondateur" ? (
            <>
              <input type="number" inputMode="decimal" value={totalDue} onChange={(e) => { setTotalDue(e.target.value); setTotalDueTouched(true); }} className="w-full px-3 py-2.5 rounded-lg outline-none" style={inputStyle} />
            ...
            <>
              <input type="number" inputMode="decimal" value={totalDue} disabled className="w-full px-3 py-2.5 rounded-lg outline-none font-semibold" style={{ ...inputStyle, background: "#FFF3D6", borderColor: "#E8B84B", color: COLORS.ink, opacity: 1 }} />
  ...
  return (
    <Modal title={t("new_expense")} onClose={onClose}>
      <div className="space-y-4">
        ...
        <div><FieldLabel>{t("amount_label")}</FieldLabel><input type="number" inputMode="decimal" value={amount} onChange={(e) => setAmount(e.target.value)} className="w-full px-3 py-2.5 rounded-lg outline-none" style={inputStyle} /></div>
  ...
  return (
    <Modal title={t("record_payment")} onClose={onClose}>
      <div className="space-y-4">
        ...
        <div><FieldLabel>{t("amount_currency")} ({currency})</FieldLabel><input type="number" inputMode="decimal" value={amount} onChange={(e) => setAmount(e.target.value)} className="w-full px-3 py-2.5 rounded-lg outline-none" style={inputStyle} /></div>
        <div><FieldLabel>{t("date_label")}</FieldLabel><input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full px-3 py-2.5 rounded-lg outline-none" style={inputStyle} /></div>
        <button onClick={submit} className="w-full py-3 rounded-lg font-semibold mt-2" style={{ background: COLORS.ink, color: "#fff" }}>{t("save")}</button>
      </div>
    </Modal>
      ...
      <div className="rounded-2xl p-5 mb-6" style={{ background: COLORS.card, border: `1px solid ${COLORS.line}` }}>
        ...
        <div className="grid sm:grid-cols-2 gap-3 mb-4">
          ...
          <div>
            ...
            <input type="number" inputMode="decimal" min={4} max={20} value={nbQuestions} onChange={(e) => setNbQuestions(e.target.value)} className="w-full px-3 py-2.5 rounded-lg outline-none text-sm" style={inputStyle} />
          </div>
        </div>
