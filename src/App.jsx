...
export default function App() {
  ...
  const loadProfile = useCallback(async (userId) => {
    const { data } = await supabase.from("profiles").select("*, schools(name, code, currency, current_year, level_type, subscribed, subscription_tier, trial_started_at, momo_number, momo_provider, logo_url, card_theme, director_title)").eq("user_id", userId).maybeSingle();
      ...
      <main className="flex-1 p-5 md:p-10 pb-20 md:pb-10 max-w-5xl mx-auto w-full">
        ...
        {tab === "grades" && !isSecondaire && (role === "enseignant" || role === "fondateur" || role === "directeur") && (
          <>
            ...
            <GradesView students={yearStudents} grades={yearGrades} releases={yearReleases} readOnly={role === "fondateur" || role === "directeur"} canPublish={role === "directeur"} lockedClass={role === "enseignant" ? profile.class_name : null} passThreshold={isSecondaire ? 10 : 5} staffProfiles={staffProfiles} onSave={saveGrade} onToggleRelease={toggleRelease} onPublishAll={publishAllReleases} schoolName={profile.schools?.name} bulletinAppreciations={bulletinAppreciations} onSaveAppreciation={saveAppreciation} directorTitle={profile.schools?.director_title} />
          </>
        )}
        {tab === "grades" && isSecondaire && (role === "enseignant" || role === "fondateur" || role === "directeur") && (
          <>
        ...
        {tab === "tuitionfees" && role === "fondateur" && (
          <TuitionFeesView levelType={levelType} tuitionFees={tuitionFees} onSave={saveTuitionFee} tierComplete={hasTierAccess(profile.schools, "complet")} schoolMomo={profile.schools} onSaveMomo={saveMomoSettings} onUploadLogo={uploadSchoolLogo} onSaveTheme={saveCardTheme} onSaveDirectorTitle={saveDirectorTitle} />
        )}
...
function TuitionFeesView({ levelType, tuitionFees, onSave, tierComplete, schoolMomo, onSaveMomo, onUploadLogo, onSaveTheme, onSaveDirectorTitle }) {
  const { t } = useLang();
...
function GradesView({ students, grades, releases, readOnly, canPublish, lockedClass, passThreshold, staffProfiles, onSave, onToggleRelease, onPublishAll, schoolName, bulletinAppreciations, onSaveAppreciation, directorTitle }) {
  const { t } = useLang();
