...
import {
  ...
  Wallet, TrendingUp, AlertTriangle, BookOpen, FileSpreadsheet, Smartphone, TrendingDown, Receipt, Shield, ClipboardList, ArrowDownWideNarrow, MessageSquare, Menu, CalendarCheck, Languages, Share2, Banknote, Contact, BookMarked,
} from "lucide-react";
...
const TRANSLATIONS = {
  fr: {
    ...
    parent_word: "Parent",
    account_deactivated_title: "Ton compte est désactivé pour cette année scolaire",
    account_deactivated_staff_body: "Contacte le fondateur/directeur de l'école pour qu'il réactive ton accès depuis \"Personnel\".",
  ...
  en: {
    ...
    parent_word: "Parent",
    account_deactivated_title: "Your account is deactivated for this school year",
    account_deactivated_staff_body: "Contact the school's founder/director so they can reactivate your access from \"Staff\".",
...
function MainApp({ profile, refreshProfile }) {
  ...
  const validateExpense = async (id) => {
    ...
    if (data) { setExpenses((prev) => prev.map((e) => (e.id === id ? data[0] : e))); notify(t("success_saved")); }
    return error;
  };
  ...
  const NAV = {
    ...
    enseignant: [["lessons", t("nav_lessons"), BookOpen], ["grades", t("nav_grades"), ClipboardList], ...(isSecondaire ? [["myassignments", t("nav_my_assignments"), BookMarked]] : []), ["attendance", t("nav_attendance"), CalendarCheck], ["students", t("nav_students"), Users], ["mysalary", t("nav_my_salary"), Banknote]],
    ...
    <div className="min-h-screen w-full flex" style={{ background: COLORS.paper, fontFamily: "'Inter', sans-serif" }}>
      ...
      <main className="flex-1 p-5 md:p-10 pb-20 md:pb-10 max-w-5xl mx-auto w-full">
        ...
        {tab === "classteachers" && role === "fondateur" && (
          <ClassTeachersView students={yearStudents} teacherAssignments={teacherAssignments} classHeadTeachers={classHeadTeachers} staffProfiles={staffProfiles} onSet={setClassHeadTeacher} onAssign={assignTeacherFondateur} onUnassign={deleteTeacherAssignment} />
        )}
        {tab === "announcements" && (
      ...
      {modal === "student" && <StudentModal levels={levels} student={editingStudent} onClose={() => { setModal(null); setEditingStudent(null); }} onSave={addStudent} onUpdate={updateStudent} />}
    ...
    <div>
      ...
      {role === "fondateur" && schoolCode && (
        <div className="rounded-xl mb-6 overflow-hidden" style={{ background: COLORS.primarySoft }}>
          ...
          <button onClick={onShare} className="w-full flex items-center justify-center gap-2 px-4 py-3 text-sm font-semibold" style={{ background: COLORS.primary, color: "#fff" }}>
...
function GradesSecondaireView({ students, gradesSecondaire, releases, allAssignments, myUserId, classHeadTeachers, subjectsSecondaire, currency, readOnly, isHeadTeacherOf, onSave, onSetCoefficient, onToggleRelease }) {
  ...
  const renderSaisie = () => {
    ...
    const exportSaisieExcel = () => {
      const rows = [[t("student"), t("written"), t("oral"), t("month_avg"), t("term_avg")]];
      classStudents.forEach((s) => {
        const g = getGrade(s.id, subject, month);
        rows.push([s.full_name, g?.ecrit ?? "", g?.oral ?? "", monthAvg(s.id) !== null ? monthAvg(s.id).toFixed(2) : "", trimAvg(s.id) !== null ? trimAvg(s.id).toFixed(2) : ""]);
      });
      const csv = rows.map((r) => r.map((v) => `"${String(v).replace(/"/g, '""')}"`).join(",")).join("\n");
      const blob = new Blob(["\uFEFF" + csv], { type: "text/csv;charset=utf-8;" });
      const url = URL.createObjectURL(blob);
...
function ClassTeachersView({ students, teacherAssignments, classHeadTeachers, staffProfiles, onSet, onAssign, onUnassign }) {
  const { t } = useLang();
  const [saving, setSaving] = useState(null);
      ...
      {allTeachers.length > 0 && (
        <div className="rounded-2xl p-5 mb-8" style={{ background: COLORS.card, border: `1px solid ${COLORS.line}` }}>
          ...
          <div className="space-y-3 mb-3">
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              <div><FieldLabel>{t("subject_label")}</FieldLabel><input value={assignSubject} onChange={(e) => setAssignSubject(e.target.value)} placeholder={t("subject_placeholder2")} className="w-full px-3 py-2.5 rounded-lg outline-none text-sm" style={inputStyle} /></div>
              <div><FieldLabel>{t("class_label")}</FieldLabel><input value={assignClass} onChange={(e) => setAssignClass(e.target.value)} placeholder={t("class_placeholder2")} className="w-full px-3 py-2.5 rounded-lg outline-none text-sm" style={inputStyle} /></div>
            </div>
          </div>
    ...
    <div>
      ...
      <div className="rounded-2xl p-5 mb-6" style={{ background: COLORS.card, border: `1px solid ${COLORS.line}` }}>
        <div className="grid sm:grid-cols-2 gap-3 mb-3">
          <div><FieldLabel>{t("subject_label")}</FieldLabel><input value={subject} onChange={(e) => setSubject(e.target.value)} placeholder="Ex: Mathématiques" className="w-full px-3 py-2.5 rounded-lg outline-none" style={inputStyle} /></div>
          <div><FieldLabel>{t("class_label")}</FieldLabel><input value={className} onChange={(e) => setClassName(e.target.value)} placeholder="Ex: 6ème A" className="w-full px-3 py-2.5 rounded-lg outline-none" style={inputStyle} /></div>
        </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody style={{ fontFamily: "'IBM Plex Mono', monospace" }}>
                ...
                <tr style={{ borderTop: `1px solid ${COLORS.line}` }}>
                  ...
                  {subjectStats.map((s) => (
                    <td key={s.subject} className="text-center px-3 py-2.5">
                      <div className="text-[10px] font-normal" style={{ color: COLORS.inkSoft }}>{s.pf}/{s.f}</div>
                      <div className="font-bold text-base" style={{ color: COLORS.primary }}>{s.pctF}%</div>
                    </td>
                  ))}
                </tr>
                <tr style={{ borderTop: `2px solid ${COLORS.line}`, background: COLORS.paper }}>
...
// ---------- Modals ----------
function StudentModal({ levels, student, onClose, onSave, onUpdate }) {
  const { t } = useLang();
  ...
  return (
    <Modal title={isEdit ? t("edit_student") : t("new_student")} onClose={onClose}>
      <div className="space-y-4">
        ...
        <div><FieldLabel>{t("class_label")}</FieldLabel><input value={className} onChange={(e) => setClassName(e.target.value)} placeholder="Ex: CM2, 6ème A..." className="w-full px-3 py-2.5 rounded-lg outline-none" style={inputStyle} /></div>
