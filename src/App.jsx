...
function MainApp({ profile, refreshProfile }) {
  ...
  const loadAll = useCallback(async () => {
    try {
      const [s, p, l, h, a, par, exp, gr, rel, staff, sp, rem, att, ta, cht, subj, grs, tf, svc, svcSub, svcPay, appr, promo, cal, disc, pdecl] = await Promise.all([
      ...
      const allSucceeded = [s, p, l, h, a, par, exp, gr, rel, staff, sp, rem, att, ta, cht, subj, grs, tf, svc, svcSub, svcPay, appr, promo, cal, disc, pdecl].every((r) => r.data);
      if (allSucceeded) {
        const snapshot = {
  ...
  const NAV = {
    ...
    comptable: [["students", t("nav_students_payments"), Users], ["paymentdeclarations", t("nav_payment_declarations"), Smartphone], ["reenrollment", t("nav_reenrollment"), ArrowRight], ["services", t("nav_services"), Package], ["dashboard", t("nav_stats"), LayoutDashboard], ["expenses", t("nav_expenses"), Receipt], ["staff", t("nav_staff"), Wallet], ["mysalary", t("nav_my_salary"), Banknote], ["calendar", t("nav_calendar"), CalendarCheck]],
    enseignant: isSecondaire
      ? [["grades", t("nav_grades"), ClipboardList], ["myassignments", t("nav_my_assignments"), BookMarked], ["attendance", t("nav_attendance"), CalendarCheck], ["lessonai", t("nav_lesson_ai"), Sparkles], ["lessons", t("nav_lessons"), BookOpen], ["discipline", t("nav_discipline"), AlertTriangle], ["students", t("nav_students"), Users], ["mysalary", t("nav_my_salary"), Banknote], ["calendar", t("nav_calendar"), CalendarCheck]]
      : [["grades", t("nav_grades"), ClipboardList], ["attendance", t("nav_attendance"), CalendarCheck], ["lessonai", t("nav_lesson_ai"), Sparkles], ["lessons", t("nav_lessons"), BookOpen], ["discipline", t("nav_discipline"), AlertTriangle], ["students", t("nav_students"), Users], ["mysalary", t("nav_my_salary"), Banknote], ["calendar", t("nav_calendar"), CalendarCheck]],
  ...
  return (
    <div className="min-h-screen w-full flex" style={{ background: COLORS.paper, fontFamily: "'Inter', sans-serif" }}>
      ...
      {modal === "student" && <StudentModal levels={levels} student={editingStudent} existingClassNames={existingClassNames} tuitionFees={tuitionFees} role={role} tierComplete={hasTierAccess(profile.schools, "complet")} onClose={() => { setModal(null); setEditingStudent(null); }} onSave={addStudent} onUpdate={updateStudent} />}
...
// ---------- Modals ----------
function StudentModal({ levels, student, existingClassNames, tuitionFees, role, tierComplete, onClose, onSave, onUpdate }) {
  const { t } = useLang();
