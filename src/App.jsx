...
import {
  ...
  Wallet, TrendingUp, AlertTriangle, BookOpen, FileSpreadsheet, Smartphone, TrendingDown, Receipt, Shield,
} from "lucide-react";
...
function MainApp({ profile }) {
  ...
  const loadAll = useCallback(async () => {
    const [s, p, l, h, a, par, exp] = await Promise.all([
  ...
  const NAV = {
    fondateur: [["dashboard", "Tableau de bord", LayoutDashboard], ["announcements", "Annonces", Megaphone], ["students", "Élèves", Users], ["expenses", "Dépenses", Receipt], ["lessons", "Devoirs", BookOpen]],
  ...
  return (
    <div className="min-h-screen w-full flex" style={{ background: COLORS.paper, fontFamily: "'Inter', sans-serif" }}>
      ...
      <main className="flex-1 p-5 md:p-10 pb-20 md:pb-10 max-w-5xl mx-auto w-full">
        {tab === "dashboard" && role === "parent" && <ParentChildrenView children={myChildren} paidByStudent={paidByStudent} currency={currency} payments={payments} onRelink={relinkSelf} />}
        ...
        {tab === "expenses" && (role === "comptable" || role === "fondateur") && (
          <ExpensesView expenses={expenses} currency={currency} readOnly={false} onAdd={() => setModal("expense")} onDelete={deleteExpense} />
        )}
        {tab === "announcements" && (
...
function ExpensesView({ expenses, currency, onAdd, onDelete }) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <AddButton onClick={onAdd} label="Ajouter" />
      </div>
...
function ParentChildrenView({ children, paidByStudent, currency, payments, onRelink }) {
...
function StudentModal({ onClose, onSave }) {
  ...
  const submit = async () => { if (!fullName.trim() || !className.trim()) return; await onSave({ full_name: fullName.trim(), level, class_name: className.trim(), total_due: parseFloat(totalDue) || 0, parent_phone: parentPhone.trim() || null }); onClose(); };
  return (
