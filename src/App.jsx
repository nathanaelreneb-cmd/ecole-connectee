...
// Remplace le fond d'une photo par une couleur unie (blanc ou rouge), en détectant
// automatiquement la silhouette de la personne — modèle léger conçu pour mobile (le même
// principe que le flou d'arrière-plan dans Google Meet). Expérimental : si ça échoue ou
// prend trop de temps sur un téléphone à mémoire limitée, on revient à la photo d'origine
// sans jamais la perdre.
async function replacePhotoBackground(file, bgColor = "#ffffff", maxDim = 640) {
  ...
  try {
    ...
    // On décode l'image directement en petite taille (comme pour la signature), pour
    // rester léger en mémoire avant même de lancer la détection.
    ...
    segmenter.setOptions({ modelSelection: 0 }); // modèle le plus léger des deux disponibles
    ...
    const resultCanvas = await new Promise((resolve, reject) => {
      ...
      segmenter.onResults((results) => {
        ...
        try {
          ...
          // Fond uni d'abord, puis on ne garde de la photo que ce que le masque désigne
      ...
      <div className="rounded-lg overflow-hidden" style={{ background: "#fff" }}>
        ...
        <div className="p-2">
          ...
          <div className="flex items-center justify-between pt-1 mt-1 shrink-0" style={{ borderTop: `1px dashed ${COLORS.line}` }}>
            <div className="text-[5px]" style={{ color: COLORS.inkSoft }}>{t("director_label")}</div>
