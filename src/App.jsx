...
import {
  ...
  Wallet, TrendingUp, AlertTriangle, BookOpen, FileSpreadsheet, Smartphone, TrendingDown, Receipt, Shield, ClipboardList, ArrowDownWideNarrow, MessageSquare, Menu, CalendarCheck, Languages, Share2, Banknote, Contact, BookMarked, QrCode, Eye, EyeOff, Sparkles, Package, Coins, Download, Printer, ArrowLeft, ArrowRight, Star,
} from "lucide-react";
...
function MainApp({ profile, refreshProfile }) {
  ...
  const loadAll = useCallback(async () => {
    try {
      const [s, p, l, h, a, par, exp, gr, rel, staff, sp, rem, att, ta, cht, subj, grs, tf, svc, svcSub, svcPay, appr, promo, cal] = await Promise.all([
      ...
      const allSucceeded = [s, p, l, h, a, par, exp, gr, rel, staff, sp, rem, att, ta, cht, subj, grs, tf, svc, svcSub, svcPay, appr, promo, cal].every((r) => r.data);
      if (allSucceeded) {
        const snapshot = {
          ...
          promotionEntries: promo.data, calendarEvents: cal.data,
        };
  ...
  const { t, lang } = useLang();
  const NAV = {
    fondateur: [["dashboard", t("nav_dashboard"), LayoutDashboard], ["students", t("nav_students"), Users], ["annualbalance", t("nav_annual_balance"), TrendingUp], ["promotion", t("nav_promotion"), ArrowRight], ["tuitionfees", t("nav_tuition_fees"), Coins], ["services", t("nav_services"), Package], ["attendance", t("nav_attendance"), CalendarCheck], ["grades", t("nav_grades"), ClipboardList], ["expenses", t("nav_expenses"), Receipt], ["staff", t("nav_staff"), Wallet], ["mysalary", t("nav_my_salary"), Banknote], ["calendar", t("nav_calendar"), CalendarCheck]],
    comptable: [["students", t("nav_students_payments"), Users], ["reenrollment", t("nav_reenrollment"), ArrowRight], ["services", t("nav_services"), Package], ["dashboard", t("nav_stats"), LayoutDashboard], ["expenses", t("nav_expenses"), Receipt], ["staff", t("nav_staff"), Wallet], ["mysalary", t("nav_my_salary"), Banknote], ["calendar", t("nav_calendar"), CalendarCheck]],
    enseignant: isSecondaire
      ? [["grades", t("nav_grades"), ClipboardList], ["myassignments", t("nav_my_assignments"), BookMarked], ["attendance", t("nav_attendance"), CalendarCheck], ["lessonai", t("nav_lesson_ai"), Sparkles], ["lessons", t("nav_lessons"), BookOpen], ["students", t("nav_students"), Users], ["mysalary", t("nav_my_salary"), Banknote], ["calendar", t("nav_calendar"), CalendarCheck]]
      : [["grades", t("nav_grades"), ClipboardList], ["attendance", t("nav_attendance"), CalendarCheck], ["lessonai", t("nav_lesson_ai"), Sparkles], ["lessons", t("nav_lessons"), BookOpen], ["students", t("nav_students"), Users], ["mysalary", t("nav_my_salary"), Banknote], ["calendar", t("nav_calendar"), CalendarCheck]],
    parent: [["dashboard", t("nav_my_children"), GraduationCap], ["revision", t("nav_revision"), Sparkles], ["lessons", t("nav_lessons"), BookOpen], ["announcements", t("nav_announcements"), Megaphone], ["remarks", t("nav_my_remarks"), MessageSquare], ["pay", t("nav_pay_remotely"), Smartphone], ["calendar", t("nav_calendar"), CalendarCheck]],
    personnel: [["mysalary", t("nav_my_salary"), Banknote], ["announcements", t("nav_announcements"), Megaphone], ["calendar", t("nav_calendar"), CalendarCheck]],
    directeur: [["grades", t("nav_grades"), ClipboardList], ["dossiers", t("nav_dossiers"), Contact], ["promotion", t("nav_promotion"), ArrowRight], ["announcements", t("nav_announcements"), Megaphone], ["attendance", t("nav_attendance"), CalendarCheck], ...(isSecondaire ? [["classteachers", t("nav_head_teachers"), Shield]] : [["classassign", t("nav_class_assign"), Shield]]), ["lessons", t("nav_lessons"), BookOpen], ["mysalary", t("nav_my_salary"), Banknote], ["remarks", t("nav_remarks"), MessageSquare], ["calendar", t("nav_calendar"), CalendarCheck]],
  }[role];
  const NAV_FULL = profile.is_admin ? [...NAV, ["admin", t("nav_admin"), Shield], ["feedback", t("nav_feedback"), Star]] : NAV;
...
function PromotionPrepView({ students, viewYear, promotionEntries, levelType, onGenerate, onUpdateEntry }) {
  const { t } = useLang();
    ...
    <div>
      ...
      {existingTargetYears.length > 0 && (
        <div className="mb-5">
          ...
          <select value={activeYear} onChange={(e) => setSelectedTargetYear(e.target.value)} className="w-full sm:w-64 px-3 py-2.5 rounded-lg outline-none text-sm" style={inputStyle}>
            ...
            <div className="space-y-2">
              {rows.map(({ entry, student }) => (
                <div key={entry.id} className="rounded-2xl p-4 flex items-center justify-between flex-wrap gap-3" style={{ background: entry.is_repeating ? "#FFF3D6" : COLORS.card, border: `1px solid ${entry.is_repeating ? "#E8B84B" : COLORS.line}` }}>
                  <div>
                    <div className="font-semibold flex items-center gap-2" style={{ color: student.gender === "Fille" ? COLORS.negative : COLORS.ink }}>
                      {student.full_name}
...
function ReenrollmentView({ students, promotionEntries, tuitionFees, currency, onEnroll }) {
  const { t } = useLang();
  const [search, setSearch] = useState("");
  ...
  const rows = pending
    ...
    .filter((r) => r.student && (!search.trim() || r.student.full_name.toLowerCase().includes(search.toLowerCase())));
  ...
  return (
    <div>
      ...
      {rows.length === 0 ? (
        ...
        <div className="space-y-2">
          {rows.map(({ entry, student }) => (
            <div key={entry.id} className="rounded-2xl p-4" style={{ background: entry.is_repeating ? "#FFF3D6" : COLORS.card, border: `1px solid ${entry.is_repeating ? "#E8B84B" : COLORS.line}` }}>
...
// ---------- Dossiers élèves — accès Directeur (aucune donnée financière) ----------
            ...
            <table className="w-full text-xs" style={{ borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ background: COLORS.ink }}>
                  <th className="text-left px-2 py-2 font-semibold" style={{ color: "#fff" }}>{t("full_name_label")}</th>
                  <th className="text-center px-2 py-2 font-semibold" style={{ color: "#fff" }}>{t("gender_col_short")}</th>
                  <th className="text-center px-2 py-2 font-semibold" style={{ color: "#fff" }}>{t("birth_date_label")}</th>
                  <th className="text-left px-2 py-2 font-semibold" style={{ color: "#fff" }}>{t("birth_place_label")}</th>
                  <th className="text-left px-2 py-2 font-semibold" style={{ color: "#fff" }}>{t("father_name_label")}</th>
                  <th className="text-left px-2 py-2 font-semibold" style={{ color: "#fff" }}>{t("mother_name_label")}</th>
                </tr>
              </thead>
              <tbody>
                {students.map((s, i) => (
                  <tr key={s.id} style={{ borderBottom: `1px solid ${COLORS.line}`, background: i % 2 === 1 ? COLORS.paper : "transparent" }}>
                    <td className="px-2 py-2 font-medium" style={{ color: COLORS.ink }}>{s.full_name}</td>
                    <td className="px-2 py-2 text-center" style={{ color: COLORS.inkSoft }}>{s.gender === "Fille" ? "F" : "M"}</td>
                    <td className="px-2 py-2 text-center" style={{ color: COLORS.inkSoft }}>{s.date_naissance ? new Date(s.date_naissance).toLocaleDateString("fr-FR") : "—"}</td>
                    <td className="px-2 py-2" style={{ color: COLORS.inkSoft }}>{s.lieu_naissance || "—"}</td>
                    <td className="px-2 py-2" style={{ color: COLORS.inkSoft }}>{s.nom_pere || "—"}</td>
                    <td className="px-2 py-2" style={{ color: COLORS.inkSoft }}>{s.nom_mere || "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            {getClassOptions(lvl).map((cName) => (
              <div key={cName} className="rounded-2xl p-4" style={{ background: COLORS.card, border: `1px solid ${COLORS.line}` }}>
                ...
                <div className="grid sm:grid-cols-2 gap-2.5 mb-3">
                  <div>
                    <FieldLabel>{t("tuition_standard_label")}</FieldLabel>
                    <input
                      type="number"
...
function StudentModal({ levels, student, existingClassNames, tuitionFees, role, onClose, onSave, onUpdate }) {
  ...
  const submit = async () => {
    ...
    const payload = {
      ...
      date_naissance: dateNaissance || null, lieu_naissance: lieuNaissance.trim() || null,
