...
export default function App() {
  ...
  const loadProfile = useCallback(async (userId) => {
    const { data } = await supabase.from("profiles").select("*, schools(name, code, currency, current_year, level_type, subscribed, subscription_tier, trial_started_at, momo_number, momo_provider)").eq("user_id", userId).maybeSingle();
...
function StudentCardPreview({ schoolName, currentYear, student, directeur, onClose }) {
  const { t } = useLang();
    ...
    <div className="fixed inset-0 z-[90] overflow-y-auto" style={{ background: "rgba(32,48,74,0.55)" }}>
      ...
      <div className="min-h-screen flex items-start justify-center p-3 sm:p-6">
        <div className="w-full max-w-sm">
          ...
          <div id="card-print-area" className="rounded-2xl overflow-hidden" style={{ background: "#fff", boxShadow: "0 10px 40px rgba(0,0,0,0.2)", border: `3px solid ${COLORS.ink}` }}>
            <div className="px-4 py-3 text-center" style={{ background: COLORS.ink }}>
            ...
            <div className="p-4">
              <div className="text-center mb-3">
                ...
                <div className="text-xs" style={{ color: COLORS.inkSoft }}>{currentYear}</div>
              </div>

              <div className="flex gap-3 mb-3">
                <div className="shrink-0 rounded-xl overflow-hidden" style={{ width: 90, height: 110, background: COLORS.paper, border: `1px solid ${COLORS.line}` }}>
                  {student.photo_url ? (
                ...
                <div className="flex-1 min-w-0 space-y-1.5">
                  <div>
                    ...
                    <div className="text-sm font-bold" style={{ color: COLORS.ink }}>{student.full_name}</div>
                  </div>
                  <div>
                    <div className="text-[9px] uppercase font-semibold" style={{ color: COLORS.inkSoft }}>{t("class_label")}</div>
                    <div className="text-sm font-semibold" style={{ color: COLORS.ink }}>{student.class_name}</div>
                  </div>
                  <div>
                    <div className="text-[9px] uppercase font-semibold" style={{ color: COLORS.inkSoft }}>{t("gender_col_short")}</div>
                    <div className="text-sm" style={{ color: COLORS.ink }}>{student.gender || "—"}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between pt-3" style={{ borderTop: `1px solid ${COLORS.line}` }}>
...
function StudentCardsView({ students, schoolName, currentYear, directeur }) {
  const { t } = useLang();
  const [search, setSearch] = useState("");
  const classNames = useMemo(() => [...new Set(students.map((s) => s.class_name).filter(Boolean))].sort(), [students]);
  const [selectedClass, setSelectedClass] = useState("");
  ...
  return (
    <div>
      ...
      {cardFor && (
        <StudentCardPreview schoolName={schoolName} currentYear={currentYear} student={cardFor} directeur={directeur} onClose={() => setCardFor(null)} />
      )}
