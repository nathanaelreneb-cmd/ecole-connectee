...
function MainApp({ profile, refreshProfile }) {
  ...
  const loadAll = useCallback(async () => {
    try {
      const [s, p, l, h, a, par, exp, gr, rel, staff, sp, rem, att, ta, cht, subj, grs, tf, svc, svcSub, svcPay, appr, promo, cal, disc] = await Promise.all([
      ...
      const allSucceeded = [s, p, l, h, a, par, exp, gr, rel, staff, sp, rem, att, ta, cht, subj, grs, tf, svc, svcSub, svcPay, appr, promo, cal, disc].every((r) => r.data);
      if (allSucceeded) {
        const snapshot = {
          ...
          promotionEntries: promo.data, calendarEvents: cal.data, disciplineNotes: disc.data,
        };
    ...
    <div className="min-h-screen w-full flex" style={{ background: COLORS.paper, fontFamily: "'Inter', sans-serif" }}>
      ...
      <main className="flex-1 p-5 md:p-10 pb-20 md:pb-10 max-w-5xl mx-auto w-full">
        ...
        {tab === "tuitionfees" && role === "fondateur" && (
          <TuitionFeesView levelType={levelType} tuitionFees={tuitionFees} onSave={saveTuitionFee} tierComplete={hasTierAccess(profile.schools, "complet")} />
        )}
...
function TuitionFeesView({ levelType, tuitionFees, onSave, tierComplete }) {
  const { t } = useLang();
