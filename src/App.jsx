...
export default function App() {
  ...
  const loadProfile = useCallback(async (userId) => {
    const { data } = await supabase.from("profiles").select("*, schools(name, code, currency, current_year, level_type, subscribed, subscription_tier, trial_started_at, momo_number, momo_provider, logo_url, emblem_url, card_theme, director_title, ire, dpe, dsee, ville)").eq("user_id", userId).maybeSingle();
