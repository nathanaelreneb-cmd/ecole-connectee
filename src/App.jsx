...
function GradesView({ students, grades, releases, readOnly, lockedClass, passThreshold, onSave, onToggleRelease }) {
  ...
  const exportExcel = async () => {
    ...
    styleDataRow(sheet.addRow(["Garçons", ...subjectStats.map((s) => `${s.pctG}%`)]));
    const fillesSubjRow = sheet.addRow(["Filles", ...subjectStats.map((s) => `${s.pctF}%`)]);
    styleDataRow(fillesSubjRow);
    fillesSubjRow.eachCell((c) => { c.font = { color: { argb: "FFB23B32" } }; });
    const totalSubjRow = sheet.addRow(["Total", ...subjectStats.map((s) => `${s.pctT}%`)]);
