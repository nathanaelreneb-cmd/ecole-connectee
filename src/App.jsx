...
export default function App() {
  ...
  const loadProfile = useCallback(async (userId) => {
    const { data } = await supabase.from("profiles").select("*, schools(name, code, currency, current_year, level_type, subscribed, subscription_tier, trial_started_at, momo_number, momo_provider, logo_url)").eq("user_id", userId).maybeSingle();
...
function MainApp({ profile, refreshProfile }) {
  ...
  const uploadSchoolLogo = async (file) => {
    ...
    try {
      if (!file || !file.type || !file.type.startsWith("image/")) { alert("Ce fichier n'est pas reconnu comme une image. Choisis une photo JPEG ou PNG."); return null; }
      if (file.size > 8_000_000) { alert("Cette image est trop lourde (max 8 Mo). Choisis-en une plus légère."); return null; }
      const ext = file.type.includes("png") ? "png" : "jpg";
    ...
    <div className="min-h-screen w-full flex" style={{ background: COLORS.paper, fontFamily: "'Inter', sans-serif" }}>
      ...
      <main className="flex-1 p-5 md:p-10 pb-20 md:pb-10 max-w-5xl mx-auto w-full">
        ...
        {tab === "tuitionfees" && role === "fondateur" && (
          <TuitionFeesView levelType={levelType} tuitionFees={tuitionFees} onSave={saveTuitionFee} tierComplete={hasTierAccess(profile.schools, "complet")} schoolMomo={profile.schools} onSaveMomo={saveMomoSettings} onUploadLogo={uploadSchoolLogo} />
        )}
...
function StudentCardPreview({ schoolName, schoolLogo, currentYear, student, directeur, onClose }) {
  const { t } = useLang();
    ...
    <div className="fixed inset-0 z-[90] overflow-y-auto" style={{ background: "rgba(32,48,74,0.55)" }}>
      ...
      <div className="min-h-screen flex items-start justify-center p-3 sm:p-6">
        <div className="w-full max-w-sm">
          ...
          <div id="card-print-area" className="rounded-2xl overflow-hidden relative" style={{ background: "#fff", boxShadow: "0 10px 40px rgba(0,0,0,0.2)" }}>
            {/* Bande colorée décorative en haut, couleur de l'école */}
            <div style={{ height: 6, background: `linear-gradient(90deg, ${COLORS.primary}, #E8B84B)` }} />
            ...
            <div className="px-4 py-3 flex items-center gap-3" style={{ background: COLORS.ink }}>
              <div className="w-10 h-10 rounded-full overflow-hidden shrink-0 flex items-center justify-center" style={{ background: "#fff" }}>
                {schoolLogo ? <img src={schoolLogo} alt="" className="w-full h-full object-contain" /> : <div className="w-full h-full rounded-full" style={{ background: `linear-gradient(135deg, ${COLORS.primary}, #E8B84B)` }} />}
              </div>
            ...
            <div className="p-4">
              <div className="text-center mb-3">
                ...
                <div className="text-xs font-medium" style={{ color: COLORS.primary }}>{currentYear}</div>
              </div>
              ...
              <div className="flex gap-3 mb-3">
                ...
                <div className="flex-1 min-w-0 space-y-1.5">
                  <div>
                    ...
                    <div className="inline-block text-sm font-bold pb-0.5" style={{ color: COLORS.ink, borderBottom: `2px solid ${COLORS.primary}` }}>{student.full_name}</div>
                  </div>
                  <div>
                    <div className="text-[9px] uppercase font-semibold" style={{ color: COLORS.inkSoft }}>{t("class_label")}</div>
                    <span className="inline-block text-xs font-bold px-2 py-0.5 rounded-full mt-0.5" style={{ background: COLORS.primarySoft, color: COLORS.primary }}>{student.class_name}</span>
                  </div>
                  <div>
            ...
            <div style={{ height: 6, background: `linear-gradient(90deg, #E8B84B, ${COLORS.primary})` }} />
          </div>
        </div>
      </div>
    </div>
...
function StudentCardsView({ students, schoolName, schoolLogo, currentYear, directeur }) {
  const { t } = useLang();
  const [search, setSearch] = useState("");
  const classNames = useMemo(() => [...new Set(students.map((s) => s.class_name).filter(Boolean))].sort(), [students]);
  const [selectedClass, setSelectedClass] = useState("");
  ...
  return (
    <div>
      <h1 className="text-3xl font-extrabold mb-1" style={{ fontFamily: "'Manrope', sans-serif", color: COLORS.ink }}>{t("nav_student_cards")}</h1>
      <p className="mb-5" style={{ color: COLORS.inkSoft }}>{t("student_cards_subtitle")}</p>

      {cardFor && (
        <StudentCardPreview schoolName={schoolName} schoolLogo={schoolLogo} currentYear={currentYear} student={cardFor} directeur={directeur} onClose={() => setCardFor(null)} />
      )}
...
function TuitionFeesView({ levelType, tuitionFees, onSave, tierComplete, schoolMomo, onSaveMomo, onUploadLogo }) {
  const { t } = useLang();
