...
function MainApp({ profile, refreshProfile }) {
  ...
  const enqueueOfflineAction = (action) => {
    setOfflineQueue((prev) => {
      const next = [...prev, { ...action, id: `${Date.now()}_${Math.random().toString(36).slice(2, 8)}` }];
      persistQueue(next);
      return next;
    });
  };
  ...
  const loadAll = useCallback(async () => {
    try {
      ...
      const allSucceeded = [s, p, l, h, a, par, exp, gr, rel, staff, sp, rem, att, ta, cht, subj, grs].every((r) => r.data);
      if (allSucceeded) {
        const snapshot = {
          ...
          classHeadTeachers: cht.data, subjectsSecondaire: subj.data, gradesSecondaire: grs.data,
        };
  ...
  const processOfflineQueue = useCallback(async () => {
    ...
    for (const action of queue) {
      try {
        const { data, error } = await supabase.from(action.table).upsert(action.payload, { onConflict: action.conflict }).select();
        if (error || !data) { remaining.push(action); continue; }
  ...
  return (
    <div className="min-h-screen w-full flex" style={{ background: COLORS.paper, fontFamily: "'Inter', sans-serif" }}>
      {isOffline && (
        <div className="fixed top-0 left-0 right-0 z-[70] flex items-center justify-center gap-2 px-4 py-2.5" style={{ background: COLORS.negative }}>
          <AlertTriangle size={15} color="#fff" />
          <span className="text-sm font-semibold" style={{ color: "#fff" }}>
            {offlineDataAge ? t("offline_showing_cached") : t("offline_banner")}
          </span>
        </div>
      )}
      {!isOffline && offlineQueue.length > 0 && (
        <div className="fixed top-0 left-0 right-0 z-[70] flex items-center justify-center gap-2 px-4 py-2.5" style={{ background: syncingQueue ? COLORS.primary : COLORS.primarySoft }}>
          {syncingQueue ? <Loader2 className="animate-spin" size={15} color="#fff" /> : <AlertTriangle size={15} style={{ color: COLORS.primary }} />}
          <span className="text-sm font-semibold" style={{ color: syncingQueue ? "#fff" : COLORS.primary }}>
