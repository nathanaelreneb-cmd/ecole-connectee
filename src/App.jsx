...
function MainApp({ profile, refreshProfile }) {
  ...
  const NAV = {
    ...
    directeur: [["announcements", t("nav_announcements"), Megaphone], ["attendance", t("nav_attendance"), CalendarCheck], ["grades", t("nav_grades"), ClipboardList], ...(isSecondaire ? [["classteachers", t("nav_head_teachers"), Shield]] : [["classassign", t("nav_class_assign"), Shield]]), ["mysalary", t("nav_my_salary"), Banknote], ["remarks", t("nav_remarks"), MessageSquare]],
  }[role];
    ...
    <div className="min-h-screen w-full flex" style={{ background: COLORS.paper, fontFamily: "'Inter', sans-serif" }}>
      ...
      <main className="flex-1 p-5 md:p-10 pb-20 md:pb-10 max-w-5xl mx-auto w-full">
        ...
        {tab === "grades" && !isSecondaire && (role === "enseignant" || role === "fondateur" || role === "directeur") && (
          <GradesView students={yearStudents} grades={yearGrades} releases={yearReleases} readOnly={role === "fondateur" || role === "directeur"} canPublish={role === "directeur"} lockedClass={role === "enseignant" ? profile.class_name : null} passThreshold={isSecondaire ? 10 : 5} onSave={saveGrade} onToggleRelease={toggleRelease} onPublishAll={publishAllReleases} />
        )}
        {tab === "grades" && isSecondaire && (role === "enseignant" || role === "fondateur" || role === "directeur") && (
        ...
        {tab === "remarks" && (role === "fondateur" || role === "directeur") && (
...
function GradesSecondaireView({ students, gradesSecondaire, releases, allAssignments, myUserId, classHeadTeachers, subjectsSecondaire, currency, readOnly, canPublish, isHeadTeacherOf, onSave, onSetCoefficient, onToggleRelease, onPublishAll }) {
  const { t } = useLang();
...
function GradesView({ students, grades, releases, readOnly, canPublish, lockedClass, passThreshold, onSave, onToggleRelease, onPublishAll }) {
  const { t } = useLang();
  ...
  const buildClassSheet = (workbook, cName, cTerm) => {
  ...
  const exportExcel = async () => {
    if (!window.ExcelJS) { alert("Le générateur Excel n'a pas pu se charger. Vérifie ta connexion et réessaie."); return; }
    const workbook = new window.ExcelJS.Workbook();
    buildClassSheet(workbook, className, term);
    const buffer = await workbook.xlsx.writeBuffer();
    const blob = new Blob([buffer], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
  ...
  const exportAllExcel = async () => {
    ...
    const workbook = new window.ExcelJS.Workbook();
    classNames.forEach((cName) => buildClassSheet(workbook, cName, term));
    const buffer = await workbook.xlsx.writeBuffer();
    const blob = new Blob([buffer], { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
