      ...
      <main className="flex-1 p-5 md:p-10 pb-20 md:pb-10 max-w-5xl mx-auto w-full">
        {tab === "dashboard" && role === "parent" && (
          <ParentChildrenView
            ...
            payments={payments}
            onRelink={relinkSelf}
            grades={[...yearGrades, ...[...new Set(students.map((s) => s.class_name))].flatMap((cn) => computeAnnualGrades(yearGrades, cn))].filter((g) =>
              yearReleases.some((r) => r.class_name === g.class_name && r.term === g.term && r.published)
            )}
        ...
        {tab === "grades" && !isSecondaire && (role === "enseignant" || role === "fondateur") && (
          <GradesView students={students} grades={yearGrades} releases={yearReleases} readOnly={role === "fondateur"} lockedClass={role === "enseignant" ? profile.class_name : null} onSave={saveGrade} onToggleRelease={toggleRelease} />
        )}
        {tab === "grades" && isSecondaire && (role === "enseignant" || role === "fondateur") && (
