...
function AdminView() {
  ...
  return (
    <div>
      ...
      <p className="mb-6" style={{ color: COLORS.inkSoft }}>{t("admin_subtitle")}</p>
...
function StudentModal({ levels, onClose, onSave }) {
  ...
  return (
    <Modal title={t("new_student")} onClose={onClose}>
      <div className="space-y-4">
        ...
        <button onClick={submit} className="w-full py-3 rounded-lg font-semibold mt-2" style={{ background: COLORS.ink, color: "#fff" }}>{t("add_student_btn")}</button>
      </div>
    </Modal>
  );
}
