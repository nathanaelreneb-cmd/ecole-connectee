...
export default async function handler(req, res) {
  ...
  const systemPrompt =
    lang === "en"
      ? `You turn a teacher's lesson plan into a simple revision sheet directly readable by a primary/secondary student in Guinea, West Africa. Use very simple, clear, encouraging language appropriate for the child's level. Respond ONLY with valid JSON, no markdown code fences, no commentary outside the JSON.`
      : `Tu transformes un plan de leçon d'enseignant en une fiche de révision simple, directement lisible par un élève de Guinée (Afrique de l'Ouest). Utilise un langage très simple, clair et encourageant, adapté au niveau de l'élève. Réponds UNIQUEMENT avec un JSON valide, sans balises markdown, sans commentaire en dehors du JSON.`;
...
${JSON.stringify(lessonPlan).slice(0, 6000)}

${
  lang === "en"
    ? `Produce a short, friendly revision sheet with a summary, 3-6 key points to remember, and 3-5 simple practice exercises with their answers. Return ONLY this JSON shape:\n${jsonShape}`
    : `Produis une fiche de révision courte et amicale avec un résumé, 3 à 6 points clés à retenir, et 3 à 5 exercices d'entraînement simples avec leurs réponses. Réponds UNIQUEMENT avec ce format JSON :\n${jsonShape}`
}
`.trim();
